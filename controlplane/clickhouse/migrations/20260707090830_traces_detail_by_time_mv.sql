...
-- Feeds `traces_detail_by_time` from otel_traces using the EXACT same transform and root-span
-- filter as `traces_mv`, so both read paths stay byte-for-byte equivalent during the migration
-- window. A TO-table MV's row order is governed by the target table's ORDER BY, so there is no
-- trailing ORDER BY here. No Timestamp cut-line: this MV ingests every root span from creation
-- forward; historical rows are backfilled separately (see
-- rfc/traces-analytics-performance/backfill_traces_detail_by_time.sh).

CREATE MATERIALIZED VIEW IF NOT EXISTS cosmo.traces_detail_by_time_mv TO cosmo.traces_detail_by_time AS
SELECT
    TraceId,
    SpanId,
...
DROP VIEW IF EXISTS cosmo.traces_detail_by_time_mv
