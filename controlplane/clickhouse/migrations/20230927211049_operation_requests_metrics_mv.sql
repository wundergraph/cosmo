...
SELECT
    ...
    sumIf(Value, position(Attributes['http.status_code'],'5') = 1 OR position(Attributes['http.status_code'],'4') = 1 OR mapContains(Attributes, 'wg.request.error')) as TotalErrors,
    sumIf(Value, position(Attributes['http.status_code'],'4') = 1 OR mapContains(Attributes, 'wg.request.error')) AS TotalClientErrors,
