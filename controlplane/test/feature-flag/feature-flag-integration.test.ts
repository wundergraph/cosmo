...
import {
  ...
  createNamespace,
  createThenPublishFeatureSubgraph, DEFAULT_NAMESPACE,
...
describe('Feature flag integration tests', () => {
  ...
  test('that a failing base federated graph rejects a feature flag until it composes successfully', getDebugTestOptions(isDebugMode), async () => {
    ...
    expect(createFeatureFlagResponse.compositionErrors[1])
  ...
  test('that a feature flag whose labels are updated recompose the correct federated graphs successfully', getDebugTestOptions(isDebugMode),  async () => {
    const { client, server, blobStorage } = await SetupTest({ dbname });
