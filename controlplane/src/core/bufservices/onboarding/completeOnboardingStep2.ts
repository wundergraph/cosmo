...
export function completeOnboardingStep2(
  opts: RouterOptions,
  req: CompleteOnboardingStep2Request,
  ctx: HandlerContext,
): Promise<PlainMessage<CompleteOnboardingStep2Response>> {
  let logger = getLogger(ctx, opts.logger);

  return handleError<PlainMessage<CompleteOnboardingStep2Response>>(ctx, logger, async () => {
    const authContext = await opts.authenticator.authenticate(ctx.requestHeader);
    logger = enrichLogger(ctx, logger, authContext);

    const onboardingRepo = new OnboardingRepository(logger, opts.db);

    const onboarding = await onboardingRepo.completeStep2({
