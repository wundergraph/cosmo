...
func (ec *executionContext) resolveEntity(
	...
	case "Country":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Country": %w`, err)
		}
		switch resolverName {

		case "findCountryByKeyName":
			id0, err := ec.unmarshalNString2string(ctx, rep["key"].(map[string]any)["name"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findCountryByKeyName(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindCountryByKeyName(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Country": %w`, err)
			}
...
func entityResolverNameForCountry(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"key\" for Country", ErrTypeNotFound))
			break
		}
		if m, ok = val.(map[string]any); !ok {
			// nested field value is not a map[string]interface so don't use it
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to nested Key Field \"key\" value not matching map[string]any for Country", ErrTypeNotFound))
			break
		}
		val, ok = m["name"]
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"name\" for Country", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Country", ErrTypeNotFound))
			break
		}
		return "findCountryByKeyName", nil
	}
	return "", fmt.Errorf("%w for Country due to %v", ErrTypeNotFound,
