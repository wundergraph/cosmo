...
func (ec *executionContext) resolveEntity(
	...
	case "Consultancy":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Consultancy": %w`, err)
		}
		switch resolverName {
		...
		case "findConsultancyByUpc":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findConsultancyByUpc(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindConsultancyByUpc(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Consultancy": %w`, err)
			}
			err = ec.PopulateConsultancyRequires(ctx, entity, rep)
			if err != nil {
				return nil, fmt.Errorf(`populating requires for Entity "Consultancy": %w`, err)
			}
			return entity, nil
		}
	case "Cosmo":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Cosmo": %w`, err)
		}
		switch resolverName {
		...
		case "findCosmoByUpc":
			id0, err := ec.unmarshalNID2string(ctx, rep["upc"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findCosmoByUpc(): %w`, err)
			}
	...
	case "Employee":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Employee": %w`, err)
		}
		switch resolverName {
		...
		case "findEmployeeByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findEmployeeByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindEmployeeByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Employee": %w`, err)
			}
			err = ec.PopulateEmployeeRequires(ctx, entity, rep)
			if err != nil {
				return nil, fmt.Errorf(`populating requires for Entity "Employee": %w`, err)
			}
			return entity, nil
		}
	case "SDK":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "SDK": %w`, err)
		}
		switch resolverName {
		...
		case "findSDKByUpc":
			id0, err := ec.unmarshalNID2string(ctx, rep["upc"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findSDKByUpc(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindSDKByUpc(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "SDK": %w`, err)
			}
...
func entityResolverNameForConsultancy(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"upc\" for Consultancy", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Consultancy", ErrTypeNotFound))
			break
		}
		return "findConsultancyByUpc", nil
	}
	return "", fmt.Errorf("%w for Consultancy due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForCosmo(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"upc\" for Cosmo", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Cosmo", ErrTypeNotFound))
			break
		}
		return "findCosmoByUpc", nil
	}
	return "", fmt.Errorf("%w for Cosmo due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForEmployee(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Employee", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Employee", ErrTypeNotFound))
			break
		}
		return "findEmployeeByID", nil
	}
	return "", fmt.Errorf("%w for Employee due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForSDK(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"upc\" for SDK", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for SDK", ErrTypeNotFound))
			break
		}
		return "findSDKByUpc", nil
	}
	return "", fmt.Errorf("%w for SDK due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
