...
// Content width for a 120-char cli-table3 column: 120 - 2 (left + right padding)
const TABLE_CONTENT_WIDTH = 118;
...
describe('Error table rendering with large text (#2619)', () => {
    ...
    for (const error of errors) {
      ...
      for (const line of wrapped.split('\n')) {
        if (line.trim().length > 0) {
          expect(line.length).toBeLessThanOrEqual(TABLE_CONTENT_WIDTH);
        }
      }
    }
  ...
  // This test documents the cli-table3 bug. If it starts failing (fixed path
  // is no longer faster), cli-table3 may have fixed their wordWrap and our
  // CLITable wrapper could potentially be removed.
