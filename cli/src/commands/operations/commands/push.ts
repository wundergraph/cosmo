...
const parseOperationsJson = (data: any): string[] => {
    ...
    if (Array.isArray(data) && data.length === data.filter((x) => Array.isArray(x) && x.length === 2 && typeof x[0] === 'string' && typeof x[1] === 'string').length) {
...
export default (opts: BaseCommandOptions) => {
    ...
    command.requiredOption(
        ...
        'The client identifier to register the operations to',
    );
    command.requiredOption(
        '-f, --file <file>',
