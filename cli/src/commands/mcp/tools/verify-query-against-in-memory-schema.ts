        ...
        async ({ query, schema: schemaString }) => { // Renamed schema to schemaString to avoid conflict
            try {
                ...
                try {
                    ...
                    return {
                        content: [
                            { type: "text", text: `Query parsing failed:\n${syntaxError.message}` }
                        ]
                    };
                }
                ...
                try {
                    ...
                    return {
                        content: [
                            { type: "text", text: `Schema building failed:\n${schemaError.message}` }
                        ]
                    };
                }
                ...
                if (validationErrors.length > 0) {
                    const errorMessages = validationErrors.map((error: GraphQLError) => {
                        const locations = error.locations?.map(loc => `line ${loc.line}, column ${loc.column}`).join(', ') || 'unknown location';
                        return `- ${error.message} (at ${locations})`;
                    }).join('\n');
                    return {
                        content: [
                            { type: "text", text: `Query validation failed:\n${errorMessages}` }
                        ]
                    };
                }
                ...
                return {
                    content: [
                        { type: "text", text: "Query is valid against the schema." }
                    ]
                };
                ...
                return {
                    content: [
                        { type: "text", text: `An unexpected error occurred: ${error.message}` }
                    ]
                };
            }
        }
