...
if (isSentryFeatureReplayEnabled) {
  integrations.push(
    replayIntegration({
      ...
      // capped at max 15s
      minReplayDuration: parseFloat(process.env.NEXT_PUBLIC_SENTRY_REPLAY_MIN_REPLAY_DURATION || '15'),
      maskAllText: true,
      blockAllMedia: true,
    }),
  );
...
init({
   ...
   * all events)
   */
  sampleRate: parseFloat(process.env.NEXT_PUBLIC_SENTRY_CLIENT_SAMPLE_RATE || '0'),
  /**
   ...
   * ignored.
   */
  tracesSampleRate: parseFloat(process.env.NEXT_PUBLIC_SENTRY_CLIENT_TRACES_SAMPLE_RATE || '0'),
  /**
  ...
  replaysOnErrorSampleRate: isSentryFeatureReplayEnabled
    ? parseFloat(process.env.NEXT_PUBLIC_SENTRY_CLIENT_REPLAYS_ON_ERROR_SAMPLE_RATE || '0')
    : 0,
  /**
  ...
  replaysSessionSampleRate: isSentryFeatureReplayEnabled
    ? parseFloat(process.env.NEXT_PUBLIC_SENTRY_CLIENT_REPLAYS_SESSION_SAMPLE_RATE || '0')
    : 0,
  integrations,
});
