...
import {
  buildStateToShare,
  createCompressedStateUrl,
} from "@/lib/playground-url-state-encoding";
...
const WARNING_MESSAGES = {
  URL_TOO_LONG: {
    title: "Warning!",
    description:
      "The generated URL is too long and may not work in all browsers. Consider removing some options.",
  },
};

const DEFAULT_SELECTED_OPTIONS = SHARE_OPTIONS.reduce(
  (acc, { id, isChecked }) => {
    acc[id] = isChecked;
    return acc;
  },
...
export const useSharePlaygroundModal = (isOpen: boolean) => {
  ...
  const [scriptsTabState] = useLocalStorage<{
    [key: string]: Record<string, any>;
  }>("playground:script:tabState", {});
  const [preFlightScript] = useLocalStorage<any>(
    "playground:pre-flight:selected",
    null,
  );

  // Helper to check if a script is valid
  const isValidScript = (script: any) =>
    script &&
      ...
      if (!isDisabled) {
        switch (option.id) {
          ...
          case OPTION_TYPES.PRE_OPERATION: {
            const script = currentActiveTab.id
              ? scriptsTabState[currentActiveTab.id]?.["pre-operation"]
              : null;
          ...
          case OPTION_TYPES.POST_OPERATION: {
            const script = currentActiveTab.id
              ? scriptsTabState[currentActiveTab.id]?.["post-operation"]
              : null;
            isDisabled = !currentActiveTab.id || !isValidScript(script);
            break;
          }
  ...
  const generateShareableUrl = useCallback(
    (options: Record<ShareOptionId, boolean>) => {
      try {
        ...
        toast({
          ...
          description:
            "We couldn't generate the shareable URL. Please try again later",
        });
