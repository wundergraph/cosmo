        ...
        <div className="space-y-1.5">
          ...
          <CardDescription className="text-sm text-muted-foreground">
            <ol className="list-decimal px-4 pt-2">
              <li>Your account will be permanently deleted. This action is irreversible!</li>
              <li>You will lose memberships to all your organizations.</li>
              <li>
                In case of organizations where you are the only member, the organizations and all containing resources
                will be deleted.
              </li>
              <li>Any API keys created by you in all organizations will be deleted.</li>
            </ol>
          </CardDescription>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button variant="destructive" className="w-full md:ml-auto md:w-max">
              Delete account
            </Button>
          </DialogTrigger>
          <DialogContent>
            ...
            <form onSubmit={handleSubmit(handleDeleteUser)} className="mt-2">
              <div className="flex flex-col gap-y-3">
                ...
                {errors.email && <span className="px-2 text-xs text-destructive">{errors.email.message}</span>}
                <span className="mt-2 text-sm">
                  Enter <strong>DELETE MY ACCOUNT</strong> to confirm.
                </span>
                <Input type="text" {...register('confirmation')} />
                {errors.confirmation && (
                  <span className="px-2 text-xs text-destructive">{errors.confirmation.message}</span>
                )}
...
ManageAccountPage.getLayout = (page) => {
  return getDashboardLayout(page, 'Manage Account', 'Manage your account settings and preferences.');
};
