...
import { CheckCircledIcon, ChevronDownIcon, Component2Icon, GitHubLogoIcon, ReaderIcon } from '@radix-ui/react-icons';
...
export const ProposalDetails = ({
  ...
  const { mutate: approveProposal, isPending: isApproving } = useMutation(updateProposal, {
    onSuccess: (data) => {
  ...
  const { mutate: closeProposal, isPending: isClosing } = useMutation(updateProposal, {
    onSuccess: (data) => {
      if (data.response?.code === EnumStatusCode.OK) {
        toast({
  ...
  const currentSubgraph = currentSubgraphs.find((subgraph) => subgraph.name === activeSubgraphName);
  const currentSubgraphSdl = currentSubgraph?.schemaSDL || '';

  const { name, createdAt, createdByEmail, state, subgraphs, latestCheckSuccess, origin } = proposal;
    ...
    <div className="flex h-full flex-col">
            ...
            <dd>
              <div className="flex items-center gap-x-2">
                <Badge
                  variant="outline"
                  className={cn('gap-2 py-1.5', {
                    'border-success/20 bg-success/10 text-success hover:bg-success/20': latestCheckSuccess,
                    'border-destructive/20 bg-destructive/10 text-destructive hover:bg-destructive/20':
                      !latestCheckSuccess,
                  })}
                >
                ...
                <DropdownMenuContent align="end" className="w-[500px] p-3">
                  <div className="flex flex-col space-y-4 px-2 py-1">
                    <div className="flex border-b">
                      <span className="text-md mb-3 font-medium text-muted-foreground">Finish your review</span>
                    </div>
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          !latestCheckSuccess && 'cursor-not-allowed opacity-50',
                        )}
                        ...
                        <div className="grid gap-1.5">
                          <Label
                            ...
                            className={cn(
                              ...
                              !latestCheckSuccess ? 'cursor-not-allowed text-muted-foreground' : '',
                            )}
                          >
                            Approve
                          </Label>
                          <p className="text-sm text-muted-foreground">Approve the changes made by the proposal.</p>
                        </div>
                      </div>
                      ...
                      <div
                        ...
                        onClick={(e: React.MouseEvent) => e.stopPropagation()}
                      >
                        <RadioGroupItem value="CLOSED" id="close-option" className="h-[14px] w-[14px]" />
                        <div className="grid gap-1.5">
                          <Label htmlFor="close-option" className="cursor-pointer font-semibold">
                            Close
                          </Label>
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="h-full flex-1">
          <Tabs value={tab ?? 'schemas'} className="flex h-full min-h-0 flex-col">
            <div className="flex flex-row px-4 py-4 lg:px-6">
              <TabsList>
                <TabsTrigger value="schemas" className="flex items-center gap-x-2" asChild>
                  <Link href={{ query: { ...router.query, tab: 'schemas' } }}>
                    <ReaderIcon className="flex-shrink-0" />
                    Proposed Schemas
                  </Link>
                </TabsTrigger>
                <TabsTrigger value="checks" className="flex items-center gap-x-2" asChild>
            ...
            <div className="flex min-h-0 flex-1">
              <TabsContent value="schemas" className="relative w-full flex-1">
                {subgraphs.length === 0 ? (
                  <EmptyState icon={<ExclamationTriangleIcon />} title="No subgraph schemas in this proposal. " />
                ) : (
                    ...
                    <div className="-top-[60px] right-8 px-5 md:absolute md:px-0">
                      <div className="flex gap-x-2">
                              ...
                              {subgraphs.map((sg) => {
                                return (
                                  <SelectItem key={sg.name} value={sg.name}>
                                    ...
                                    >
                                      <p>{sg.name}</p>
                                      {sg.isDeleted && <p className="text-xs">(deleted)</p>}
                                    </div>
                                  </SelectItem>
                                );
                              })}
                        ...
                        {!activeSubgraph?.isDeleted && (
                          <SDLViewerActions sdl={activeSubgraphSdl} size="icon" targetName={activeSubgraphName} />
                        )}
                      </div>
                    </div>
                    <SDLViewerMonaco schema={currentSubgraphSdl} newSchema={activeSubgraphSdl} />
                  </div>
                )}
              </TabsContent>
              <TabsContent value="checks" className="relative w-full flex-1">
                {isChecksLoading ? (
                  <Loader />
                ) : checksError || checksData?.response?.code !== EnumStatusCode.OK ? (
                  <EmptyState
                    icon={<ExclamationTriangleIcon />}
                    title="Could not retrieve checks for this proposal"
                    description={checksData?.response?.details || checksError?.message || 'Please try again'}
                  />
                ) : (
                  <div className="flex h-full flex-col gap-y-3 px-4">
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            {graphData?.graph?.supportsFederation && <TableHead>Subgraph</TableHead>}
                    ...
                    <Pagination
                      limit={limit}
                      noOfPages={Math.ceil((checksData.totalChecksCount || 0) / limit)}
...
const ProposalDetailsPage: NextPageWithLayout = () => {
  ...
  if (isLoading) {
  ...
  } else if (error || !data || data?.response?.code !== EnumStatusCode.OK || !data.proposal) {
    content = (
      <EmptyState
        icon={<ExclamationTriangleIcon />}
    ...
    content = <ProposalDetails proposal={data.proposal} refetch={refetch} currentSubgraphs={data.currentSubgraphs} />;
  }
