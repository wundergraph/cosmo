...
import { DecorationCollection, SDLViewerMonaco } from '@/components/schema/sdl-viewer-monaco';
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button className="flex-shrink-0 space-x-2" variant="secondary" size="sm">
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This action cannot be undone. This will forcefully mark the check as successful.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>Cancel</AlertDialogCancel>
          <AlertDialogAction onClick={props.onSubmit}>Force Success</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  );
...
const ProposedSchemas = ({
  ...
  return (
    <div className="scrollbar-custom h-full min-h-[300px] w-full overflow-auto">
      <SDLViewerMonaco
        schema={(checkedSubgraphs.length > 0 ? sdlData?.proposedSchema : sdl) || ''}
        disablePrettier
        decorationCollections={getDecorationCollection(lintIssues, graphPruningIssues)}
      ...
      <div className="right-8 top-0 px-4 md:absolute md:px-0">
        <div className="flex gap-x-2">
            ...
            >
              ...
              <SelectContent>
                <SelectGroup>
                  ...
                  {checkedSubgraphs.map(({ subgraphName: name, id, isDeleted, isNew }) => {
                    return (
          ...
          <SDLViewerActions
            sdl={(checkedSubgraphs.length > 0 ? sdlData?.proposedSchema : sdl) || ''}
            size="icon"
...
const getDecorationCollection = (
  ...
  for (const l of lintIssues) {
    ...
    decorationCollection.push({
      ...
      options: {
        ...
        inlineClassName: 'underline decoration-destructive decoration-wavy cursor-pointer z-50',
  ...
  for (const g of graphPruningIssues) {
    ...
    decorationCollection.push({
      ...
      options: {
        hoverMessage: {
          value: `${g.message}. (Rule: ${g.graphPruningRuleType ? g.graphPruningRuleType : ''})`,
        },
        inlineClassName: `underline ${
          g.severity === LintSeverity.error ? 'decoration-destructive' : 'decoration-warning'
...
const CheckDetails = ({ data, refetch }: { data: GetCheckSummaryResponse; refetch: () => void }) => {
  const graphContext = useContext(GraphContext);
  const router = useRouter();
  ...
  const currentAffectedGraph = data.affectedGraphs.find((graph) => graph.id === graphContext.graph?.id);
            ...
            <dd className="whitespace-nowrap">
              {data.check.checkedSubgraphs.length > 1
                ...
                : data.check.isDeleted ||
                    (data.check.checkedSubgraphs.length === 1 && data.check.checkedSubgraphs[0].isDeleted)
                  ? 'Delete subgraph'
                  : data.check.checkedSubgraphs.length === 1 && data.check.checkedSubgraphs[0].isNew
                    ? 'New subgraph'
                    : 'Update schema'}
            </dd>
          </div>
          ...
          <div className="flex-start flex max-w-[200px] flex-1 flex-col gap-1 ">
            ...
            <dd className="whitespace-nowrap text-sm">
              <Tooltip>
                ...
                <TooltipContent>{formatDateTime(new Date(data.check.timestamp))}</TooltipContent>
              </Tooltip>
            </dd>
          </div>
        </dl>
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <dl className="flex flex-shrink-0 flex-col space-y-6 overflow-hidden border-b px-4 py-4 lg:block lg:min-h-full lg:w-[240px] lg:overflow-auto lg:border-b-0 lg:border-r lg:px-6 xl:w-[260px] xl:px-8">
          <div className="col-span-3 flex flex-col">
            ...
            <dd className="flex flex-row flex-wrap gap-2 lg:flex lg:flex-col">
              ...
              >
                ...
                <InfoTooltip>
                  Describes if the proposed schema can be composed with all other subgraphs in the federated graph.
                </InfoTooltip>
              </Badge>
              ...
              >
                ...
                <InfoTooltip>
                  Describes if the proposed schema is free of changes that break existing client operations.
                </InfoTooltip>
              </Badge>
              ...
              <Badge
                ...
                className={cn(
                  'flex items-center space-x-1.5 py-2',
                  data.check?.clientTrafficCheckSkipped && 'text-muted-foreground',
                )}
              >
                {data.check?.clientTrafficCheckSkipped ? (
                  <>
                    ...
                    <InfoTooltip>
                      Describes if the proposed schema affects any client operations based on real usage data. You
                      skipped this check.
                    </InfoTooltip>
                  </>
                ) : (
                  <>
                    ...
                    <span className="flex-1 truncate">Operations</span>
                    <InfoTooltip>
                      Describes if the proposed schema affects any client operations based on real usage data.
                    </InfoTooltip>
                  </>
                )}
              </Badge>
              ...
              >
                {data.check?.lintSkipped ? (
                  <>
                    ...
                    <InfoTooltip>
                      Indicates if the proposed schema contains linting errors. Enable linting to see lint issues.
                    </InfoTooltip>
                  </>
                ) : (
                  <>
                    ...
                    <span className="flex-1 truncate">Lint Errors</span>
                    <InfoTooltip>Indicates if the proposed schema contains linting errors.</InfoTooltip>
                  </>
                )}
              </Badge>
              ...
              >
                {data.check?.graphPruningSkipped ? (
                  <>
                    ...
                    <InfoTooltip>
                      Indicates if the proposed schema contains graph pruning errors. Enable graph pruning linter to see
                  ...
                  <>
                    ...
                    <span className="flex-1 truncate">Pruning Errors</span>
                    <InfoTooltip>Indicates if the proposed schema contains graph pruning errors.</InfoTooltip>
                  </>
                )}
              </Badge>
                ...
                >
                  {!data.check?.proposalMatch ? (
                    <>
                      ...
                      <InfoTooltip>Indicates if the proposed schema matches a proposal.</InfoTooltip>
                    </>
                  ) : (
                    <>
                ...
                >
                  {!data.check?.checkExtensionDeliveryId ? (
                    <>
                      ...
                      <InfoTooltip>
                        Indicates whether the subgraph check extension completed successfully. Enable subgraph check
                        extensions to see the results.
                      </InfoTooltip>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="flex-1 truncate">Extension</span>
                      <InfoTooltip>Indicates whether the subgraph check extension completed successfully.</InfoTooltip>
                    </>
                  )}
                </Badge>
              )}
          ...
          {currentAffectedGraph && (
            <div className="flex flex-col">
              <dt className="mb-2 text-sm text-muted-foreground">Timeframe checked</dt>
              <dd className="flex items-center gap-x-2 text-sm">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span className="truncate whitespace-nowrap">
                      {formatDate(subDays(new Date(data.check.timestamp), currentAffectedGraph.trafficCheckDays), {
                        dateStyle: 'short',
                      })}
                    </span>
                  </TooltipTrigger>
                  <TooltipContent>
                    {formatDateTime(subDays(new Date(data.check.timestamp), currentAffectedGraph.trafficCheckDays))}
                  </TooltipContent>
                </Tooltip>
                -
                <Tooltip>
          ...
          {data.check.checkedSubgraphs.length === 1 && data.check.checkedSubgraphs[0].isNew && (
            <div className="flex flex-col">
              <dt className="mb-2 text-sm text-muted-foreground">Labels of new subgraph</dt>
              <dd className="flex items-center gap-x-2 text-sm">
                {data.check.checkedSubgraphs[0].labels.length === 0 ? (
                  <div className="italic">
                    <Tooltip delayDuration={200}>
                      <TooltipTrigger>No labels passed</TooltipTrigger>
                      <TooltipContent>Only graphs with empty label matchers will compose this subgraph</TooltipContent>
                    </Tooltip>
                  </div>
                ) : (
                  data.check.checkedSubgraphs[0].labels.map(({ key, value }) => {
                    return (
            ...
            <>
              {vcsContext.author && (
                <div className="flex flex-col">
                  ...
                  <dd className="flex items-center gap-x-2 text-sm">{vcsContext.author}</dd>
                </div>
              )}
              {vcsContext.commitSha && (
                <div className="flex flex-col">
                  <dt className="mb-2 text-sm text-muted-foreground">Commit sha</dt>
                  <dd className="flex items-center gap-x-2 text-sm">{vcsContext.commitSha}</dd>
                </div>
              )}
              {vcsContext.branch && (
                <div className="flex flex-col">
                  <dt className="mb-2 text-sm text-muted-foreground">Branch</dt>
                  <dd className="flex items-center gap-x-2 text-sm">{vcsContext.branch}</dd>
                </div>
              )}
            </>
          )}
        ...
        <div className="scrollbar-custom h-full flex-1 overflow-auto">
          <Tabs value={tab ?? 'overview'} className="flex h-full min-h-0 flex-col">
            <div className="flex flex-row px-4 py-4 lg:px-6">
              <TabsList className="justify-start overflow-x-auto scrollbar-none">
                <TabsTrigger value="overview" className="flex items-center gap-x-2" asChild>
                  <Link href={{ query: { ...router.query, tab: 'overview' } }}>
                    <CheckCircleIcon className="h-4 w-4 flex-shrink-0" />
                    Overview
                  </Link>
                </TabsTrigger>
                <TabsTrigger value="composition" className="flex items-center gap-x-2" asChild>
                  <Link
                    href={{
                  ...
                  >
                    ...
                    {data.compositionErrors.length ? (
                      <Badge variant="muted" className="bg-white px-1.5 text-current dark:bg-gray-900/60">
                        {data.compositionErrors.length + data.compositionWarnings.length}
                      </Badge>
                    ) : null}
                  </Link>
                </TabsTrigger>
                <TabsTrigger value="changes" className="flex items-center gap-x-2" asChild>
                  <Link href={{ query: { ...router.query, tab: 'changes' } }}>
                    <UpdateIcon />
                    Changes{' '}
                    {data.changes.length || data.composedSchemaBreakingChanges?.length ? (
                      <Badge variant="muted" className="bg-white px-1.5 text-current dark:bg-gray-900/60">
                        {data.changes.length + (data.composedSchemaBreakingChanges?.length || 0)}
                      </Badge>
                    ) : null}
                  </Link>
                </TabsTrigger>
                <TabsTrigger value="operations" className="flex items-center gap-x-2" asChild>
                  <Link href={{ query: { ...router.query, tab: 'operations' } }}>
                    <PiBracketsCurlyBold className="flex-shrink-0" />
                    Operations
                  </Link>
                </TabsTrigger>
                <TabsTrigger value="lintIssues" className="flex items-center gap-x-2" asChild>
                  <Link href={{ query: { ...router.query, tab: 'lintIssues' } }}>
                ...
                <TabsTrigger value="graphPruningIssues" className="flex items-center gap-x-2" asChild>
                  <Link
                    href={{
                ...
                {proposalsFeature?.enabled && (
                  <TabsTrigger value="proposalMatches" className="flex items-center gap-x-2" asChild>
                    <Link
                      href={{
                ...
                {(data.check.checkedSubgraphs.length > 1 ||
                  (data.check.checkedSubgraphs.length === 1 && !data.check.checkedSubgraphs[0].isDeleted) ||
                  (data.check.checkedSubgraphs.length === 0 && !data.check.isDeleted)) && (
                  <TabsTrigger
                    value="schema"
                ...
                {subgraphCheckExtensionsFeature?.enabled && (
                  <TabsTrigger value="checkExtension" className="flex items-center gap-x-2" asChild>
                    <Link
                      href={{
            ...
            <div className="flex min-h-0 flex-1">
              <TabsContent value="overview" className="w-full space-y-4 px-4 lg:px-6">
                <div className="space-y-4">
                  <div className="flex flex-col space-y-4">
                    <Alert variant={isSuccessful ? 'success' : 'destructive'}>
                      ...
                      <AlertTitle>{isSuccessful ? 'Check Passed' : 'Check Failed'}</AlertTitle>
                      <AlertDescription>
                        {(() => {
                          const linkedCheckFailures = [];
                          if (isLinkedTrafficCheckFailed) {
                            linkedCheckFailures.push('client traffic check failures');
                          }
                          ...
                          if (reason === 'All tasks were successful' && !isSuccessful) {
                  ...
                  {data.affectedGraphs.length > 1 && (
                    <div className="space-y-4 pt-4">
                      <div className="flex items-center gap-2">
                        <h3 className="text-lg font-semibold">Other affected graphs</h3>
                        <InfoTooltip tooltipContentClassName="w-96">
                          These are other federated graphs that also contain the subgraph being checked. Since they
                          share the same subgraph, the schema changes impact these graphs too, and this check applies to
                          them as well.
                        </InfoTooltip>
                      </div>
                      <TableWrapper>
                          ...
                          <TableBody>
                            {data.affectedGraphs.map(
                              ({
                                ...
                                if (federatedGraphId === graphContext.graph?.id) {
                                  return null;
                                }
                                ...
                                const graphPruningSkipped = data.check?.graphPruningSkipped;
                                ...
                                return (
                                  ...
                                  >
                                    <TableCell>
                                      {getCheckBadge(isCheckSuccessful, data.check?.isForcedSuccess || false)}
                                    </TableCell>
                                    ...
                                    <TableCell>
                                      <div className="flex flex-wrap items-start gap-2">
                                        ...
                                        <Badge
                                          ...
                                          className={cn(
                                            'gap-2 py-1.5',
                                            clientTrafficCheckSkipped && 'text-muted-foreground',
                                          )}
                                        >
                  ...
                  {data.check.linkedChecks.length > 0 && (
                    <div className="space-y-4 pt-4">
                      <div className="flex items-center gap-2">
                        ...
                        <InfoTooltip tooltipContentClassName="w-96">
                          These are checks performed on subgraphs that are linked to the current subgraph. The traffic
                          and pruning checks of these linked subgraphs influence the result of the current check. These
                          checks are automatically run whenever the current subgraph is checked.
                        </InfoTooltip>
                      </div>
                      <TableWrapper>
                        <Table>
                          ...
                          <TableBody>
                            {data.check.linkedChecks.map((linkedCheck) => {
                              ...
                              return (
                                ...
                                >
                                  <TableCell>
                                    {getCheckBadge(linkedCheck.isCheckSuccessful, linkedCheck.isForcedSuccess)}
                                  </TableCell>
                                  ...
                                  <TableCell>
                                    <Badge
                                      ...
                                      className={cn(
                                        'gap-2 py-1.5',
                                        linkedCheck.clientTrafficCheckSkipped && 'text-muted-foreground',
                                      )}
                                    >
                                      {linkedCheck.clientTrafficCheckSkipped ? (
                                        <NoSymbolIcon className="h-4 w-4" />
                                      ) : (
                                        getCheckIcon(!linkedCheck.hasClientTraffic)
                                      )}
                                      <span>Operations</span>
                                    </Badge>
                                    <Badge
                                      ...
                                      className={cn(
                                        'gap-2 py-1.5',
                                        linkedCheck.graphPruningCheckSkipped && 'text-muted-foreground',
                                      )}
                                    >
                                      {linkedCheck.graphPruningCheckSkipped ? (
                                        <NoSymbolIcon className="h-4 w-4" />
                                      ) : (
                                        getCheckIcon(!linkedCheck.hasGraphPruningErrors)
                                      )}
                                      <span className="flex-1 truncate">Pruning Errors</span>
                                    </Badge>
                                  </TableCell>
              ...
              <TabsContent value="composition" className="w-full space-y-4 px-4 lg:px-6">
                {data.check?.compositionSkipped ? (
                  <EmptyState
                  ...
                  <>
                    ...
                    !data.check.isComposable ? (
                      ...
                      />
                    ) : data.compositionErrors.length === 0 && data.compositionWarnings.length === 0 ? (
                      <EmptyState
              ...
              <TabsContent value="changes" className="w-full space-y-4 px-4 lg:px-6">
                {data.check?.breakingChangesSkipped ? (
                  <EmptyState
                    icon={<NoSymbolIcon className="text-gray-400" />}
                  ...
                  <>
                    {data.check.isBreaking && data.check.isComposable && data.check.hasClientTraffic ? (
                      <Alert variant="default">
                        ...
                        <AlertTitle>{data.check.isForcedSuccess ? 'Forced Success' : 'Checks Failed'}</AlertTitle>
                        <AlertDescription>
                          {data.check.isForcedSuccess ? (
                            <>This check was manually marked as successful.</>
                          ) : (
                            <>
                              The proposed schema changes can be composed, but there are breaking changes affecting
                              client operations.
                              <br />
                              You can manually override the state of this check to accept the changes.
                            </>
                          )}
                        </AlertDescription>
                        {data.check.isForcedSuccess ? (
                    ...
                    {data.changes.length > 0 || data.composedSchemaBreakingChanges.length > 0 ? (
                      <>
                        {data.changes.length > 0 && (
                          <>
                            <h3 className="text-md mb-2 font-medium">Subgraph Schema Changes</h3>
                            <ChangesTable
                        ...
                        {data.composedSchemaBreakingChanges.length > 0 ? (
                          <div className={data.changes.length > 0 ? 'mt-6' : ''}>
                            <h3 className="text-md mb-2 font-medium">Federated Graph Schema Breaking Changes</h3>
                            <p className="mb-4 text-sm text-muted-foreground">
                              These breaking changes were detected in the composed federated graph schema after
                              composition. They are not reported above because they only become visible when all
                              subgraphs are composed together (e.g., field type or nullability conflicts between
                              subgraphs).
                            </p>
              ...
              <TabsContent value="lintIssues" className="w-full space-y-4 px-4 lg:px-6">
                <LintIssuesTable
              ...
              <TabsContent value="graphPruningIssues" className="w-full space-y-4 px-4 lg:px-6">
              ...
              {proposalsFeature?.enabled && (
                <TabsContent value="proposalMatches" className="w-full space-y-4 px-4 lg:px-6">
              ...
              {subgraphCheckExtensionsFeature?.enabled && (
                <TabsContent value="checkExtension" className="w-full space-y-4 px-4 lg:px-6">
