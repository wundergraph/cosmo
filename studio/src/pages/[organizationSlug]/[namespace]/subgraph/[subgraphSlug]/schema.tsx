...
const SubgraphSchemaPage: NextPageWithLayout = () => {
  ...
  if (error || !data || data.response?.code !== EnumStatusCode.OK) {
    return (
      <SubgraphPageLayout title="Schema" subtitle="View the SDL of your subgraph" noPadding>
        <EmptyState
    ...
    <SubgraphPageLayout
      ...
      toolbar={
        <Toolbar className="w-auto flex-nowrap py-0">
          ...
          <SDLViewerActions sdl={data?.sdl ?? ''} size="icon-sm" targetName={name} />
        </Toolbar>
      }
    >
      {data.sdl === '' ? (
        ...
        <div className="flex h-full flex-col-reverse md:flex-col">
          ...
          <div className="flex w-full flex-col items-center gap-x-8 gap-y-1 border-t bg-card p-2 text-xs lg:flex-row">
            <p className="text-center">Displaying the latest published schema of this subgraph</p>
            <p className="flex items-center gap-x-1 lg:ml-auto">
              Routing URL :
              <Link className="hover:underline" target="_blank" rel="noreferrer" href={routingURL}>
                {routingURL}
              </Link>
            </p>
            {graph.subgraph.lastUpdatedAt && (
              <p className="flex items-center gap-x-1">
                Last updated :<span>{formatDateTime(new Date(graph.subgraph.lastUpdatedAt))}</span>
              </p>
            )}
          </div>
        </div>
