...
const DeleteIntegration = ({ id, refresh }: { id: string; refresh: () => void }) => {
...
const Integration = ({
  ...
  const { mutate: create, isPending: isCreating } = useMutation(createIntegration);

  const { mutate: update, isPending: isUpdating } = useMutation(updateIntegrationConfig);
  ...
  const [existingEvents, setExistingEvents] = useState<string[]>(existing?.events || []);
      ...
      },
      {
        onSuccess: (d) => {
          if (d.response?.code === EnumStatusCode.OK) {
            ...
            toast({
              description: d.response?.details ?? 'Could not create integration. Please try again.',
              duration: 3000,
            });
          }
        ...
        },
        {
          onSuccess: (d) => {
            if (d.response?.code === EnumStatusCode.OK) {
              ...
              toast({
                description: d.response?.details ?? 'Could not update integration. Please try again.',
                duration: 3000,
              });
            }
          },
  ...
  return (
    ...
    >
      ...
      >
        <DialogHeader>
          <DialogTitle>{mode === 'create' ? 'Set up a' : 'Update'} slack integration</DialogTitle>
          <DialogDescription>A message will be sent to the provided slack channel.</DialogDescription>
        </DialogHeader>

        {mode === 'create' && (
          <Form {...createForm}>
            <form className="flex w-full flex-col gap-y-6" onSubmit={createForm.handleSubmit(onSubmitOfCreate)}>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <CreateIntegrationInput placeholder="slack_channel_name" {...field} />
                    </FormControl>
                    <FormDescription>An unique name to identify the integration.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
        ...
        {mode === 'update' && (
          <Form {...updateForm}>
            <form className="flex w-full flex-col gap-y-6" onSubmit={updateForm.handleSubmit(onSubmitOfUpdate)}>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <CreateIntegrationInput placeholder="https://hooks.slack.com/..." {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
    ...
    <div className="flex flex-col gap-y-6">
      ...
      <TableWrapper>
        <Table>
          ...
          <TableBody>
            {data.integrations.map(({ id, name, events, eventsMeta, integrationConfig }) => {
              return (
                <TableRow key={id}>
