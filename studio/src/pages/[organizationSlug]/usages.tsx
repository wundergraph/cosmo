...
const FeatureLimit = ({ id, fallback, multiplier }: { id: string; fallback?: number; multiplier?: number }) => {
...
export const CustomBarChart = ({ data }: { data: { usage: number; capacity?: number; name: string }[] }) => {
  return (
    <ResponsiveContainer width="100%" height={175} className="my-auto text-xs">
      <BarChart layout="vertical" data={data}>
        <XAxis type="number" domain={[0, (data[0].capacity || 0) + data[0].usage]} tickFormatter={valueFormatter} />
        <YAxis type="category" hide />
        <Bar dataKey="usage" stackId="a" fill="#82ca9d" name="Usage" />
        {data[0].capacity ? <Bar dataKey="capacity" stackId="a" fill="#8884d8" name="Capacity" /> : null}
        <ChartTooltip formatter={valueFormatter} labelFormatter={() => 'Requests'} />
        <Legend />
      </BarChart>
    </ResponsiveContainer>
  );
...
const UsagesPage: NextPageWithLayout = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery(getOrganizationRequestsCount);
  ...
  const chartData: { usage: number; capacity?: number; name: string }[] = [
    {
      name: 'Requests',
      usage: requestLimit > 0 && Number(data.count) > requestLimit ? 0 : Number(data.count),
      capacity:
        requestLimit < 0 ? undefined : Number(data.count) > requestLimit ? 0 : requestLimit - Number(data.count),
    },
  ];
      ...
      {requestLimit > 0 && currentUsagePct === 1 ? (
        <div className="flex items-center gap-x-2 rounded-lg border !border-destructive px-4 py-2 text-destructive">
          ...
          <span>
            ...
            Please{' '}
            <a className="underline underline-offset-2" href={calURL} target="_blank" rel="noreferrer">
              contact us
            </a>{' '}
            to upgrade.
          </span>
        ...
        <div className="flex items-center gap-x-2 rounded-lg border !border-amber-400 px-4 py-2 text-amber-400">
          ...
          <span>
            ...
            Please{' '}
            <a className="text-amber-500 underline underline-offset-2" href={calURL} target="_blank" rel="noreferrer">
              contact us
            </a>{' '}
            to upgrade.
          </span>
        ...
        <Card className="col-span-2 flex flex-col gap-y-3 p-3">
          <div className="flex items-center gap-x-2">
            ...
            <span className="text-xs text-muted-foreground">
              {`${requestLimit === -1 ? 'Unlimited' : formatMetric(requestLimit)} / month`}
            </span>
          </div>
        ...
        <Card className="col-span-1 flex flex-col">
          ...
          <CardContent className="px-4 pt-0">
            <dl className="space-y-2">
              <div className="flex">
                <dt className="flex-1 px-2 text-sm text-muted-foreground">Users</dt>
                <dd className="w-1/3 px-2 text-right text-sm font-medium">
                  <FeatureLimit id="users" fallback={25} />
                </dd>
              </div>
              <div className="flex">
                <dt className="flex-1 px-2 text-sm text-muted-foreground">Federated graphs & Monographs</dt>
                <dd className="w-1/3 px-2 text-right text-sm font-medium">
              ...
              <div className="flex">
                <dt className="flex-1 px-2 text-sm text-muted-foreground">Tracing Data Retention</dt>
                <dd className="w-1/3 px-2 text-right text-sm font-medium">
                  <FeatureLimit id="tracing-retention" fallback={30} /> days
                </dd>
              </div>
              <div className="flex">
                <dt className="flex-1 px-2 text-sm text-muted-foreground">Breaking Changes Retention</dt>
                <dd className="w-1/3 px-2 text-right text-sm font-medium">
                  <FeatureLimit id="breaking-change-retention" fallback={30} /> days
                </dd>
              </div>
              <div className="flex">
                <dt className="flex-1 px-2 text-sm text-muted-foreground">Changelog Data Retention</dt>
                <dd className="w-1/3 px-2 text-right text-sm font-medium">
                  <FeatureLimit id="changelog-retention" fallback={30} /> days
                </dd>
              </div>
              <div className="flex">
                <dt className="flex-1 px-2 text-sm text-muted-foreground">Field Grace Period Limit</dt>
                <dd className="w-1/3 px-2 text-right text-sm font-medium">
                  <FeatureLimit id="field-pruning-grace-period" fallback={30} /> days
                </dd>
              </div>
              <div className="flex">
                <dt className="flex-1 px-2 text-sm text-muted-foreground">Trace Sampling Rate</dt>
                <dd className="w-1/3 px-2 text-right text-sm font-medium">
                  <FeatureLimit id="trace-sampling-rate" multiplier={100} fallback={1} />%
                </dd>
              </div>
            </dl>
          </CardContent>
...
const IncreaseLimits = () => {
  const slug = useRouteParam('organizationSlug');
