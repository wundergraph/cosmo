  ...
  if (featureSubgraphs.length === 0) {
        ...
        actions={
          <CLISteps
            steps={[
              ...
              {
                description: 'Update your feature subgraphs of this feature flag.',
    ...
    content = (
      <EmptyState
        icon={<ExclamationTriangleIcon className="text-destructive" />}
        title="Feature flag is not used for composition."
        description={<>Feature flag is disabled. To enable the feature flag, use the below command.</>}
        actions={<CLI command={`npx wgc feature-flag enable <feature-flag-name> --namespace <namespace>`} />}
      />
    );
  } else if (federatedGraphs.find((f) => f.federatedGraph.name === slug)?.isConnected) {
    ...
    content = (
      <EmptyState
        ...
        description={
          <>
            The labels of this feature flag match to that of this federated graph. But to be used for composition, the
            feature subgraphs which are part of this feature flag should have their respective base subgraphs be a part
            of this federated graph.{' '}
          </>
        }
        actions={[]}
      />
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        <div className="h-full flex-1">
          <Tabs value={slug ? (tab ?? 'overview') : (tab ?? 'graphs')} className="flex h-full min-h-0 flex-col">
            <div className="flex flex-row px-4 py-4 lg:px-6">
              <TabsList>
                {!slug && (
                  <TabsTrigger value="graphs" className="flex items-center gap-x-2" asChild>
                ...
                <TabsTrigger value="featureSubgraphs" className="flex items-center gap-x-2" asChild>
                  <Link
                    href={{
                  ...
                  <div className="px-6">
                    {federatedGraphs.length > 0 ? (
                      ...
                      <EmptyState
                        ...
                        description={
                          <>
                            To associate a federated graph with this feature flag, please try updating the labels of the
                            feature flag to match the required federated graphs.{' '}
                          </>
                        }
                        actions={[]}
                      />
