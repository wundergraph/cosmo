        ...
        description={
          <>
            ...
            <a
              target="_blank"
              rel="noreferrer"
              href={docsBaseURL + '/cli/feature-subgraph/create-feature-subgraph'}
  ...
  return (
    <div className="flex flex-col gap-y-6">
      <Alert>
        ...
        <AlertDescription>
          Adding members directly to the subgraph have been deprecated. Use the groups instead.
        </AlertDescription>
      </Alert>
  ...
  return (
    <div className="flex items-center justify-end px-2">
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild disabled={!isAdmin && !(creatorUserId && creatorUserId === user?.id)}>
          <div>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Add users to <span className="italic">{subgraphName}</span> subgraph
            </DialogTitle>
          </DialogHeader>
  ...
  return (
    <Tabs value={tab ?? 'subgraphs'} className="flex min-h-0 flex-col">
      <div className="flex flex-row">
        <TabsList>
          <TabsTrigger value="subgraphs" className="flex items-center gap-x-2" asChild>
            <Link href={{ query: { ...router.query, tab: 'subgraphs', page: 1 } }}>
          ...
          <TableBody>
            {subgraphs.map(({ id, name, routingURL, lastUpdatedAt, labels, namespace, baseSubgraphName, type }) => {
              ...
              if (router.asPath.split('/')[3] === 'graph') {
                ...
                analyticsPath += `?filterState=${encodeURIComponent(JSON.stringify(query))}`;
              }
              return (
                <TableRow
                  key={name}
                ...
                >
                  ...
                  <TableCell className="flex justify-end gap-2">
                    <Tooltip delayDuration={200}>
                      <TooltipTrigger asChild>
                        <Button asChild variant="ghost" size="icon-sm" className="table-action">
                          <Link onClick={(e) => e.stopPropagation()} href={analyticsPath}>
