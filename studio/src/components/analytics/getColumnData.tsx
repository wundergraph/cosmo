      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="overflow-hidden text-ellipsis">{data.toString().substring(0, 7)}</div>
          </TooltipTrigger>
          <TooltipContent>
            <span>{data}</span>
          </TooltipContent>
  ...
  if (type === Unit.UnixTimestamp) {
    return (
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <span>{formatInTimeZone(Number(data) * 1000, 'UTC', 'MMM dd yyyy HH:mm:ss')}</span>
          </TooltipTrigger>
          <TooltipContent>
            <span>{data}</span>
          </TooltipContent>
  ...
  return (
    <Dialog2 isOpen={props.isOpen} setIsOpen={props.setIsOpen}>
      <Dialog2Content className="max-w-2xl">
        ...
        {unit === Unit.CodeBlock ? <QueryContent query={content} /> : <TextContent text={content} />}
      </Dialog2Content>
    </Dialog2>
  );
};

const ActionsDropdown = ({ actionColumns, rowData }: { actionColumns: AnalyticsViewColumn[]; rowData: any }) => {
  ...
  return (
    <>
      ...
      {data && isOpen && <ActionDialog {...data} isOpen={isOpen} setIsOpen={setIsOpen} />}
    </>
  );
};
...
const DialogActions = ({ text, className }: { text: string; className?: string }) => {
  ...
  return (
    <div className={cn('flex w-full items-center gap-x-2 md:ml-auto md:w-auto', className)}>
...
export const getColumnData = (columnData: AnalyticsViewColumn[]): ColumnDef<any>[] => {
  ...
  const columns: ColumnDef<any>[] = compact(
    columnData.map((each) => {
      ...
      return {
        ...
        cell: ({ row }) => {
          return (
            <div className={cn(config?.cell?.className)}>{formatColumnData(row.getValue(each.name), each.unit!)}</div>
          );
        },
  ...
  if (actionColumns.length) {
    const actionColumn: ColumnDef<any> = {
      ...
      cell: ({ row }) => {
        ...
        return <ActionsDropdown actionColumns={actionColumns} rowData={rowData} />;
      },
    };
