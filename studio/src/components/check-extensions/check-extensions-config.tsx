...
export type SubgraphCheckExtensionsConfig = Omit<
  PlainMessage<ConfigureSubgraphCheckExtensionsRequest>,
  "namespace"
>;
...
export function CheckExtensionsConfig({
  ...
  const [forceShowSecretKeyInput, setForceShowSecretKeyInput] = useState<
    boolean | undefined
  >(undefined);
  ...
  const onSaveChangesCallback = (newConfig: SubgraphCheckExtensionsConfig) => {
    if (
      isSecretKeyAssigned &&
      forceShowSecretKeyInput &&
      !newConfig.secretKey?.trim().length
    ) {
  ...
  const toggleableOptions = useMemo<ToggleableOptionsType[]>(
    () => [
      ...
      },
      {
        ...
        docsLink:
          "/studio/subgraph-check-extensions#include-lint-warnings-and-errors",
      ...
      },
      {
        ...
        description: isGraphPruningEnabledForNamespace ? (
          ...
          <>
            ...
            for the namespace to be able to receive graph pruning warnings and
            errors.
          </>
        ),
        docsLink:
          "/studio/subgraph-check-extensions#include-graph-pruning-warnings-and-errors",
      ...
      },
      {
        ...
        description:
          "Lists the operations that may be impacted by changes to the subgraph schema.",
        docsLink:
          "/studio/subgraph-check-extensions#include-affected-operations",
      },
    ],
    [
      isLintingEnabledForNamespace,
    ...
    <>
      <AlertDialog open={showConfirmationDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogDescription>
            You are about to update the webhook configuration and remove the
            secret key. Are you sure you want to do this?
          </AlertDialogDescription>
          <AlertDialogFooter>
            <Button
              variant="secondary"
            ...
            <CardContent className="space-y-1.5 border-t pt-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      The endpoint that will receive POST requests with event
                      data. Must be a valid absolute URL starting with{" "}
                  ...
                  <FormItem>
                    ...
                    {showSecretKeyInput ? (
                      ...
                      <div className="flex h-9 w-full items-center justify-start gap-x-1 rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm hover:border-input-active dark:bg-black">
                        <span>
                          If you have lost or forgotten this secret key, you can
                          change it.
                        </span>
                        <Button
                          variant="link"
                    ...
                    <FormDescription>
                      ...
                      <a
                        ...
                        href={
                          docsBaseURL +
                          "/studio/subgraph-check-extensions#verification"
                        }
                        target="_blank"
                        className="text-primary"
                      >
              ...
              <div className="space-y-1 py-2 text-sm">
                ...
                <p className="text-muted-foreground">
                  Specifies which data elements should be included in the
                  generated file delivered via the webhook.{" "}
                  <a
                    href={
                      docsBaseURL +
                      "/studio/subgraph-check-extensions#configuration"
                    }
                    target="_blank"
                    rel="noreferrer"
