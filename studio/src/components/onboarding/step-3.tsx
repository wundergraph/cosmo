...
function getDemoGraphStatus({
  ...
  if (data?.response?.code !== undefined && data.response.code !== EnumStatusCode.OK) {
  ...
  const hasProducts = subgraphs.some((s) => s.name === 'products');
  const hasReviews = subgraphs.some((s) => s.name === 'reviews');

  if (hasProducts && hasReviews) {
    return 'ok';
  }
    ...
    <OnboardingContainer>
      <div className="mt-4 flex w-full flex-col gap-6 text-left">
        ...
        <div className="flex gap-3">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-2">
            <p className="text-sm text-muted-foreground">
              Run this command to create a demo federated graph with sample subgraphs.
            </p>
