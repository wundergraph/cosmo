...
export const CodeViewerActions = ({
  ...
  const downloadSDL = () => {
    downloadStringAsFile(code, `${subgraphName}.${extension}`, `application/${extension}`);
  };
  ...
  return (
    <div className={cn('flex w-full items-center gap-x-2 md:w-auto', className)}>
      <Button variant={variant} size={size} className="flex-1" onClick={() => copySDL()}>
        <ClipboardCopyIcon className="mr-3" />
        Copy
      </Button>
      <Button variant={variant} size={size} className="flex-1" onClick={downloadSDL}>
