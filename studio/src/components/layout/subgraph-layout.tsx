...
export const SubgraphLayout = ({ children }: LayoutProps) => {
  ...
  if (isLoading) {
    ...
    render = <SubgraphContext.Provider value={subgraphContextData}>{children}</SubgraphContext.Provider>;
  }
...
export const getSubgraphLayout = (page: React.ReactNode, { title }: any = {}) => {
  return (
    <SubgraphLayout>
