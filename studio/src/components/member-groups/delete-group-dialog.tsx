...
const groupIdValidator = z.string().uuid({ message: 'Please select a valid group.' });
...
export function DeleteGroupDialog({
  ...
  const schema = z.object({
    ...
    toGroupId: group?.membersCount && otherGroups.length > 0 ? groupIdValidator : groupIdValidator.optional(),
  });
  ...
  return (
    <Dialog open={!!group && open} onOpenChange={handleOnOpenChange}>
      ...
      <DialogContent>
        ...
        <Form {...deleteForm}>
          <form className="mt-1 flex flex-col gap-y-4" onSubmit={deleteForm.handleSubmit(onSubmit)}>
            {group?.membersCount || group?.apiKeysCount || group?.hasOidcMappers ? (
              <>
                <span>
                  <span className="font-semibold">Before deleting</span> the group, you must select a new group. This is
                  because:
                </span>
                ...
                <ol className="ml-8 list-disc space-y-2">
                  {group.hasOidcMappers && (
                    <li>
                      One or more OIDC mapper targets this group, we need to update the mappers so when user sign in
                      using SSO they don&apos;t lose access to the organization.
                    </li>
                  )}
                  {!!group.membersCount && (
                    <li>
                      {group.membersCount === 1 ? 'One member ' : 'Multiple members '} have been assigned to this group.
                    </li>
                  )}
                  {!!group.apiKeysCount && (
                    <li>
                      {group.apiKeysCount === 1 ? 'One API Key' : 'Multiple API Keys'} have been assigned to this group.
                    </li>
                  )}
                </ol>
            ...
            <div className="space-y-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Input className="w-full" type="text" autoFocus disabled={isPending} {...field} />
                    </FormControl>
