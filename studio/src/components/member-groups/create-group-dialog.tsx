...
export function CreateGroupDialog({ onGroupCreated }: { onGroupCreated(group: OrganizationGroup): Promise<void> }) {
  ...
  const createGroupInputSchema = z.object({
    ...
    description: z.string().trim().max(500, { message: 'Description must be a maximum of 500 characters' }),
  });
      ...
      <DialogContent>
        ...
        <form className="mt-4 flex flex-col gap-y-3" onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col gap-y-2">
            <label htmlFor="create-group-name" className="text-sm font-semibold">
              Name <span className="text-destructive">*</span>
            </label>
            <Input id="create-group-name" className="w-full" disabled={isPending} type="text" {...register('name')} />

            {errors.name && <span className="px-2 text-xs text-destructive">{errors.name.message}</span>}
          </div>

          <div className="flex flex-col gap-y-2">
            <label htmlFor="create-group-description" className="text-sm font-semibold">
              Description
            </label>
            <Textarea
            ...
            {errors.description && <span className="px-2 text-xs text-destructive">{errors.description.message}</span>}
          </div>

          <Button type="submit" disabled={!isValid || isPending} variant="default" isLoading={isPending}>
            New Group
          </Button>
        </form>
      </DialogContent>
