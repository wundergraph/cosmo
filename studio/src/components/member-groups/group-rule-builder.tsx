    ...
    <div className="grid grid-cols-2 items-start justify-start gap-3">
      <div className="gap-y-20">
        <Popover open={isPopoverOpen} onOpenChange={setPopoverOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" disabled={disabled || !rbac?.enabled || builtin} className="w-full justify-start">
              <span className={cn('truncate', !activeRole && 'text-muted-foreground')}>
                {activeRole ? (
                  <span className="flex items-center justify-start gap-x-1">
                    <span>{capitalize(activeRole.category).replace('-', ' ')}</span>
          ...
          >
            {!isMobile ? (
              ...
              <GroupRolesAccordion rolesByCategory={roleContext.rolesByCategory} onSelectRole={onSelectRole} />
            )}
          </PopoverContentWithScrollableContent>
        </Popover>
      </div>
      ...
      <div className="flex items-start justify-start gap-x-2">
        ...
        {rbac?.enabled && !builtin && (
          <Button variant="ghost" size="icon" className="shrink-0" disabled={disabled} onClick={onRemoveRule}>
