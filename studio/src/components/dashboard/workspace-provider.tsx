...
export function WorkspaceProvider({ children }: React.PropsWithChildren) {
  ...
  const [namespace, setNamespace] = useState(
    namespaceParam || storedNamespace || DEFAULT_NAMESPACE_NAME,
  );
  ...
  useEffect(() => {
    ...
    if (
      !currentNamespaces.some(
        (ns) => ns.toLowerCase() === actualNamespace.toLowerCase(),
      )
    ) {
      ...
      const ns =
        currentNamespaces.find((n) => n === DEFAULT_NAMESPACE_NAME) ||
        currentNamespaces[0];
      if (ns) {
  ...
  const setNamespaceCallback = useCallback(
    (ns: string, applyRouteParams: boolean) => {
      if (
        !ns ||
        namespace === ns ||
        !namespaces.some((ns) => ns.toLowerCase() === ns.toLowerCase())
      ) {
        return;
      }
