...
import { MinusIcon, PlusIcon, UpdateIcon, DotFilledIcon } from '@radix-ui/react-icons';
...
const structureChangelogs = (changes: FederatedGraphChangelog[]): StructuredChangelog[] => {
  let parentNodeName = '';
...
const Changes = ({ changes }: { changes: FederatedGraphChangelog[] }) => {
  ...
  const getIcon = (code: string) => {
    ...
    return <DotFilledIcon className="text-muted-foreground" width={25} height={25} />;
  };
  ...
  return (
    <div className="flex flex-col gap-y-2 pt-4 lg:pt-0">
      {structuredChangelogs.map(({ changeType, parentName, childName }, index) => {
        ...
        return (
          ...
          >
            ...
            <span className="hidden text-xs italic text-muted-foreground md:block">{noCase(changeType)}</span>
          </div>
        );
      })}
    </div>
