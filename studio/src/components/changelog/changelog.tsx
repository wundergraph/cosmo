...
export const Changelog = ({ entries }: { entries: FederatedGraphChangelogOutput[] }) => {
  ...
  return (
    <ol className="relative w-full">
      {entries.map(({ schemaVersionId: id, createdAt, changelogs, compositionId }) => {
        return (
          <li id={id} key={id} className="flex w-full flex-col gap-y-8 py-10 first:pt-2">
            ...
            <div className="flex w-full flex-col items-start gap-x-16 gap-y-4 lg:flex-row">
              <div className="flex flex-col items-end gap-y-1">
                ...
                <p className="text-sm font-bold text-muted-foreground">{id.slice(0, 6)}</p>
                <div>
                  <div className="flex items-center gap-x-1">
                    <PlusIcon className="text-success" />
                    <p className="text-sm text-success">{getDiffCount(changelogs).addCount}</p>
                  </div>
                  <div className="flex items-center gap-x-1">
                    <MinusIcon className="text-destructive" />
                    <p className="text-sm text-destructive">{getDiffCount(changelogs).minusCount}</p>
                  </div>
                </div>
              </div>
