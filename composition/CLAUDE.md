...
6. `wgc` (cli workspace) — composes from composition's built dist; no separate bundle step
...
The `router-tests/entitycaching/testdata/config.json` is generated by a Go tool using the
wgc compose path (composition built dist):
...
3. Regenerate router-tests config: `cd router-tests/entitycaching && make compose`
...
cross-package workflow (composition + shared + router-tests).
