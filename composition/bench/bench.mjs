...
const { federateSubgraphs, parse, printSortedSdl } = await import(pathToFileURL(COMPOSITION_DIST).href).catch((err) => {
...
function runOnce() {
  ...
  if (result.success && process.env.BENCH_NO_EMIT !== '1') {
    sdl =
      typeof printSortedSdl === 'function'
        ? printSortedSdl(result.federatedGraphAST)
        : schemaToSortedNormalizedString(result.federatedGraphSchema);
  }
...
if (process.env.BENCH_SKIP_GOLDEN !== '1') {
  ...
  if (normalizeString(warm.sdl) !== golden) {
