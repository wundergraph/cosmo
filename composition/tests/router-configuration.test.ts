...
describe('Router Configuration tests', () => {
  describe('Normalization tests', () => {
    ...
    test('that external fields that are part of a key FieldSet are not included in the root node #1', () => {
    ...
    test('that external fields that are not part of a key FieldSet are not included in the root node #2', () => {
      const { errors, normalizationResult } = normalizeSubgraphFromString(`
      type Entity @key(fields: "id") {
        id: ID! @external
