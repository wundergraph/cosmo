...
// main refactored the free function batchNormalize() into the BatchNormalizer class;
// keep the old call shape for these tests (version is accepted and ignored).
const batchNormalize = (params: any) => new BatchNormalizer(params).batchNormalize();
import {
...
describe('Entity cache mapping rules tests', () => {
  ...
  describe('input-object mappings', () => {
    ...
    test('rule 39: an extra non-key argument globally invalidates all auto-mappings, including earlier nested-key results', () => {
      ...
      const { warnings } = normalizeSubgraphSuccess(subgraph(sdl), version);
      // One warning per invalidated key: the nested key warns first, then the flat key.
      expect(warnings).toHaveLength(2);
      expect(warnings[0].message).toBe(
        autoMappingAdditionalNonKeyArgumentWarningMessage('store', 'Query.product', 'store.id', 'Product', 'extra'),
      );
...
describe('Review regressions (PR #2944)', () => {
  test('composite @openfed__is plus a plain argument named after a mapped leaf key field errors instead of crashing', () => {
    const { errors } = normalizeSubgraphFailure(
      subgraph(`
        input ProductKey {
          id: ID!
        ...
        type Product @key(fields: "id sku") @key(fields: "region") @openfed__entityCache(maxAge: 60) {
          id: ID!
          sku: String!
          region: String!
          name: String!
        ...
        type Query {
          product(
            key: ProductKey! @openfed__is(fields: "id sku")
            regionArg: String! @openfed__is(fields: "region")
            id: ID!
          ): Product @openfed__queryCache(maxAge: 30)
        }
      `),
  ...
  test('root field cache directives are extracted from renamed root operation types', () => {
    ...
    expect(config!.rootFieldCacheConfigurations).toBeDefined();
    expect(config!.rootFieldCacheConfigurations).toHaveLength(1);
    expect(config!.rootFieldCacheConfigurations![0].fieldName).toBe('product');
  });

  test('nested input-object auto-mapping with an extra non-key argument emits the additional-argument warning', () => {
    const { warnings } = normalizeSubgraphSuccess(
      subgraph(`
        type Location {
          id: ID!
        ...
        type Warehouse @key(fields: "location { id }") @openfed__entityCache(maxAge: 60) {
          location: Location!
          name: String!
        }
        ...
        type Query {
          warehouse(location: LocationInput!, extra: String!): Warehouse @openfed__queryCache(maxAge: 30)
        }
      `),
      version,
    );
    ...
    expect(warnings).toHaveLength(1);
    expect(warnings[0].message).toBe(
      autoMappingAdditionalNonKeyArgumentWarningMessage(
        'location',
        'Query.warehouse',
        'location.id',
        'Warehouse',
