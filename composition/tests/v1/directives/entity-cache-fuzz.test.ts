...
describe('Entity caching fuzz tests', () => {
  // ═══════════════════════════════════════════════════════════════════════════
  // @queryCache + @is mapping edge cases
  // ═══════════════════════════════════════════════════════════════════════════

  describe('@queryCache + @is edge cases', () => {
    test('1. @queryCache on field with NO arguments — should succeed with empty mappings', () => {
      // A Query field with @queryCache but zero arguments cannot construct a cache key.
      // Expect: succeeds (cache population still works), empty mappings.
      const config = getConfigForType(
        subgraph(`
          type Query {
            latestProduct: Product @queryCache(maxAge: 30)
          }
    ...
    test('2. @queryCache on field returning nullable entity (Product not Product!) — should succeed', () => {
    ...
    test('3. @queryCache on field returning union type — should error (union is not an entity)', () => {
      // A union type itself doesn't have @key, so it's not an entity.
      // Expect: error about non-entity return type.
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query {
            item(id: ID!): Item @queryCache(maxAge: 30)
          }
          ...
          type Service @key(fields: "id") @entityCache(maxAge: 60) {
            id: ID!
            description: String!
          }
        `),
    ...
    test('4. @queryCache on field returning interface type — should error (interface is not a keyed entity)', () => {
      // An interface can't have @key in federation v1-style, so it's not an entity.
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query {
            node(id: ID!): Node @queryCache(maxAge: 30)
          }
          interface Node {
            id: ID!
          }
          type Product implements Node @key(fields: "id") @entityCache(maxAge: 60) {
            id: ID!
            name: String!
          }
        `),
    ...
    test('5. @is(fields: "id") on argument WITHOUT @queryCache — should error', () => {
      // @is only makes sense for cache key construction. Without @queryCache, it's meaningless.
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query {
    ...
    test('6. @is(fields: "id id") — duplicate field reference in @is', () => {
      // The @is value "id id" references the same key field twice.
      // This might be treated as a composite @is spec (contains space).
      // Expect: either an error about duplicate mapping or a parse failure.
      // Let's see what actually happens.
      const sg = subgraph(`
        type Query {
          product(key: ProductKey! @is(fields: "id id")): Product @queryCache(maxAge: 30)
        }
        input ProductKey {
          id: ID!
        }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // @entityCache edge cases
  // ═══════════════════════════════════════════════════════════════════════════

  describe('@entityCache edge cases', () => {
    test('7. @entityCache on type with multiple @key directives — config should still have one entityCacheConfig', () => {
      // Multiple @key directives on same type, single @entityCache. The entity has
    ...
    test('8. @entityCache with very large maxAge (999999999999) — should accept', () => {
      // Very large TTL. INT_MAX in JS is safe up to 2^53, so 999999999999 should be fine.
      // Expect: succeeds, config has the exact value.
      const config = getConfigForType(
        subgraph(`
          type Query { product(id: ID!): Product }
          type Product @key(fields: "id") @entityCache(maxAge: 999999999999) {
            id: ID!
            name: String!
          }
        `),
      ...
      expect(config).toBeDefined();
      expect(config!.entityCacheConfigurations).toHaveLength(1);
      expect(config!.entityCacheConfigurations![0].maxAgeSeconds).toBe(999999999999);
    });

    test('9. @entityCache on type with ONLY unresolvable @key(resolvable: false) — should succeed', () => {
      // An entity with only unresolvable keys still has @key, so @entityCache should be accepted.
    ...
    test('10. Multiple @entityCache on same type (repeated directive) — only first is used', () => {
      // @entityCache is NOT marked repeatable, so GraphQL validation should reject this.
      // But if it gets past parsing, only the first directive is processed.
      // Let's see what happens.
      const sg = subgraph(`
        type Query { product(id: ID!): Product }
        type Product @key(fields: "id") @entityCache(maxAge: 60) @entityCache(maxAge: 120) {
          id: ID!
          name: String!
        }
      `);
      ...
      if (result.success) {
        ...
        // If it succeeds, the second @entityCache should be silently ignored
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // Multiple @key + @queryCache mapping edge cases
  // ═══════════════════════════════════════════════════════════════════════════

  describe('multiple @key + @queryCache mapping', () => {
    test('11. Two @key directives, argument satisfies both — both keys should produce mappings', () => {
      ...
      const config = getConfigForType(
        subgraph(`
          ...
          type Product @key(fields: "id") @key(fields: "id sku") @entityCache(maxAge: 60) {
            id: ID!
            sku: String!
            name: String!
          }
  ...
  describe('nested key edge cases', () => {
    test('13. @key(fields: "store { id }") with @is(fields: "store.id") on scalar arg', () => {
      // Nested key path: "store.id" — the @is targets it with dot notation.
    ...
    test('14. @key with deeply nested field (3 levels)', () => {
      ...
      const { schema } = normalizeSubgraphSuccess(
        subgraph(`
          ...
          type Thing @key(fields: "a { b { c } }") @entityCache(maxAge: 60) {
            a: A!
          }
          type A {
            b: B!
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // @is with input objects
  // ═══════════════════════════════════════════════════════════════════════════

  describe('@is with input objects', () => {
    test('15. @is(fields: "id sku") with input object having EXTRA fields — should error', () => {
      // Input object has "id", "sku", and "extra". The "extra" field is not a key field.
      // With explicit @is, extra non-key arguments should be an error.
      const sg = subgraph(`
        type Query {
          product(key: ProductKey! @is(fields: "id sku")): Product @queryCache(maxAge: 30)
        }
        input ProductKey {
          id: ID!
          sku: String!
    ...
    test('16. @is(fields: "id sku") with input object MISSING one field — should error', () => {
      // Input object has "id" but not "sku". The @is says it maps to both.
      ...
      `);
      const result = batchNormalize({ subgraphs: [sg], version });
      // This should produce an error — the input object can't satisfy the @is spec.
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // @requestScoped edge cases
  // ═══════════════════════════════════════════════════════════════════════════

  describe('@requestScoped edge cases', () => {
    test('17. @requestScoped requires key — missing key is a failure', () => {
      // key is mandatory.
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query {
            currentUser: User @requestScoped
            user: User @requestScoped(key: "u")
          }
    ...
    test('18. @requestScoped with ≥ 2 fields sharing the same key — no warning, l1Key subgraph-prefixed', () => {
      const { warnings } = normalizeSubgraphSuccess(
        subgraph(`
          type Query {
            me: User @requestScoped(key: "me")
          }
          type Article @key(fields: "id") {
            id: ID!
            viewer: User @requestScoped(key: "me")
          }
          type User @key(fields: "id") {
            id: ID!
            name: String!
    ...
    test('19. @requestScoped on a non-entity type field — should succeed when ≥ 2 fields share the key', () => {
      // @requestScoped is on FIELD_DEFINITION, works on any object type field.
      const config = getConfigForType(
        subgraph(`
          type Query {
            currentLocale: String @requestScoped(key: "locale")
          }
          type Article @key(fields: "id") {
            id: ID!
            articleLocale: String @requestScoped(key: "locale")
          }
        `),
        QUERY,
      );
    ...
    test('20. @requestScoped with only one field declaring a key — warning emitted', () => {
      // Single-field @requestScoped is meaningless (no second reader to benefit).
      const { warnings } = normalizeSubgraphSuccess(
        subgraph(`
          type Query {
            currentUser: User @requestScoped(key: "lonely")
          }
          type User @key(fields: "id") {
            id: ID!
            name: String!
    ...
    test('21. Multiple @requestScoped on same field — not repeatable, should fail', () => {
      // @requestScoped is NOT repeatable.
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query {
            currentUser: User @requestScoped(key: "a") @requestScoped(key: "b")
          }
          type User @key(fields: "id") {
            id: ID!
            name: String!
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // @cacheInvalidate / @cachePopulate edge cases
  // ═══════════════════════════════════════════════════════════════════════════

  describe('@cacheInvalidate/@cachePopulate edge cases', () => {
    test('22. @cachePopulate(maxAge: -1) — should error', () => {
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query { dummy: String! }
          type Mutation {
            createProduct(name: String!): Product @cachePopulate(maxAge: -1)
          }
          type Product @key(fields: "id") @entityCache(maxAge: 60) {
            id: ID!
            name: String!
    ...
    test('23. @cacheInvalidate returning a list type — should succeed', () => {
      // Mutation returning [Product]! with @cacheInvalidate. The named type is still "Product".
      // getTypeNodeNamedTypeName unwraps list and NonNull wrappers.
      const { schema } = normalizeSubgraphSuccess(
        subgraph(`
          type Query { dummy: String! }
          type Mutation {
            deleteProducts(ids: [ID!]!): [Product!]! @cacheInvalidate
          }
          type Product @key(fields: "id") @entityCache(maxAge: 60) {
            id: ID!
            name: String!
    ...
    test('24. @cacheInvalidate on Subscription field — should succeed', () => {
      // The code allows both Mutation and Subscription for @cacheInvalidate.
      const config = getConfigForType(
        subgraph(`
          type Query { dummy: String! }
          type Subscription {
    ...
    test('25. @cachePopulate on Subscription returning a non-entity type — should error', () => {
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query { dummy: String! }
          type Subscription {
            eventOccurred: Event @cachePopulate
          }
          type Event {
            id: ID!
            message: String!
  ...
  describe('cross-directive interactions', () => {
    test('26. @queryCache + @entityCache on same field — @queryCache is on the Query field, @entityCache on the type', () => {
    ...
    test('27. @entityCache + @requestScoped on same type — both should be present in config', () => {
      ...
      const sg = subgraph(`
        type Query {
          user(id: ID!): User @queryCache(maxAge: 30)
          activeSession: String @requestScoped(key: "session")
        }
        type User @key(fields: "id") @entityCache(maxAge: 60) {
          id: ID!
          name: String!
          currentSession: String @requestScoped(key: "session")
        }
      `);
      const result = batchNormalize({ subgraphs: [sg], version }) as BatchNormalizationSuccess;
      expect(result.success).toBe(true);
    ...
    test('28. Both @cachePopulate and @queryCache on same field — should this be rejected?', () => {
      // @queryCache is only valid on Query fields, @cachePopulate only on Mutation/Subscription.
      // They can't be on the same field since a field can only be on one root type.
      // But what if someone puts @cachePopulate on a Query field? That's caught by rule 17.
      // What about @queryCache on a Mutation field? That's caught by rule 4.
      // So this combination is inherently impossible on the same field.
      // Let's verify the error for @queryCache on Mutation with @cachePopulate:
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query { dummy: String! }
          type Mutation {
            createProduct(name: String!): Product @queryCache(maxAge: 30) @cachePopulate
          }
          type Product @key(fields: "id") @entityCache(maxAge: 60) {
            id: ID!
            name: String!
      ...
      // Should error about @queryCache on non-Query field
      expect(errors).toHaveLength(1);
    });
  });
  ...
  describe('auto-mapping traps', () => {
    ...
    test('31. Entity with @key(fields: "type") — "type" is a valid field name, should work', () => {
      ...
      const config = getConfigForType(
        subgraph(`
          type Query {
            product(type: String!): Product @queryCache(maxAge: 30)
          }
          type Product @key(fields: "type") @entityCache(maxAge: 60) {
            type: String!
            name: String!
          }
        `),
  ...
  describe('batch / list edge cases', () => {
    test('33. List return with list argument — should produce isBatch mapping', () => {
      // products(ids: [ID!]!): [Product!]! with @queryCache — batch lookup.
      // The argument "ids" doesn't match key field "id" by name, so we need @is.
      const config = getConfigForType(
        subgraph(`
          type Query {
    ...
    test('34. List return with scalar argument — should not establish batch mapping', () => {
      // products(category: String!): [Product!]! with @queryCache — scalar arg can't batch.
  ...
  describe('config attachment edge cases', () => {
    test('36. @cachePopulate config goes to correct operation type (Mutation vs Subscription)', () => {
      // Verify that @cachePopulate on Mutation goes to "Mutation" config and
      // @cachePopulate on Subscription goes to "Subscription" config.
      ...
      // This means for Query it would be "Subscription" — but Query fields with @cachePopulate
      ...
      const config = getConfigForType(
        subgraph(`
          type Query { dummy: String! }
          type Subscription {
            newProduct: Product @cachePopulate(maxAge: 120)
          }
          type Product @key(fields: "id") @entityCache(maxAge: 60) {
            id: ID!
            name: String!
    ...
    test('37. @cacheInvalidate config uses correct entityTypeName from return type', () => {
    ...
    test('38. Multiple @queryCache fields on same Query type — all get configs', () => {
      // Two different query fields with @queryCache returning different entities.
      const config = getConfigForType(
        subgraph(`
          type Query {
            product(id: ID!): Product @queryCache(maxAge: 30)
            user(id: ID!): User @queryCache(maxAge: 45)
          }
          type Product @key(fields: "id") @entityCache(maxAge: 60) {
            id: ID!
            name: String!
          }
          type User @key(fields: "id") @entityCache(maxAge: 90) {
            id: ID!
            name: String!
          }
        `),
    ...
    test('39. Multiple mutations with different cache directives — all get correct configs', () => {
      const sg = subgraph(`
        ...
        type Mutation {
          ...
          createProduct(name: String!): Product @cachePopulate(maxAge: 30)
        }
        type Product @key(fields: "id") @entityCache(maxAge: 60) {
          id: ID!
          name: String!
  ...
  describe('potential silent corruption', () => {
    test('40. @entityCache without maxAge argument — should error (maxAge is required)', () => {
      ...
      const sg = subgraph(`
        type Query { product(id: ID!): Product }
        type Product @key(fields: "id") @entityCache {
          id: ID!
          name: String!
        }
      `);
    ...
    test('41. @queryCache maxAge as float (3.5) — should be parsed as 0 and error', () => {
      // maxAge is Int!, a float value should either fail parsing or be treated as 0.
      const sg = subgraph(`
        type Query {
          product(id: ID!): Product @queryCache(maxAge: 3.5)
        }
        type Product @key(fields: "id") @entityCache(maxAge: 60) {
          id: ID!
          name: String!
    ...
    test('42. @is on argument of non-Query field without @queryCache — error about @is without @queryCache', () => {
      // @is without @queryCache on a Mutation argument — should still error about @is placement.
      const { errors } = normalizeSubgraphFailure(
        subgraph(`
          type Query { dummy: String! }
          type Mutation {
            updateProduct(pid: ID! @is(fields: "id")): Product @cacheInvalidate
          }
          type Product @key(fields: "id") @entityCache(maxAge: 60) {
            id: ID!
            name: String!
      ...
      // Should have error about @is without @queryCache
      expect(errors).toHaveLength(1);
    });

    test('43. @queryCache on field returning entity with @key but entity has NO fields matching any argument — empty mappings, no warning', () => {
    ...
    test('44. @queryCache with very large maxAge but @entityCache with very small maxAge — no error about TTL mismatch', () => {
      ...
      const config = getConfigForType(
        subgraph(`
          type Query {
            product(id: ID!): Product @queryCache(maxAge: 3600)
          }
          type Product @key(fields: "id") @entityCache(maxAge: 1) {
            id: ID!
            name: String!
          }
        `),
    ...
    test('45. @queryCache on field returning entity that exists only in another subgraph', () => {
      // The entity type Product is defined in subgraph-b but not in subgraph-a.
      // However, subgraph-a references it as a return type and adds @queryCache.
      ...
      `);
      // Without @entityCache on Product, the mapping still produces a root field config
      // but with empty key mappings (since hasEntityCache is false).
      const config = getConfigForType(sg, QUERY);
      expect(config).toBeDefined();
      expect(config!.rootFieldCacheConfigurations).toHaveLength(1);
      // entityKeyMappings should be empty since Product lacks @entityCache
