...
<Steps>
    <Step title="Navigate to the Script Editor">
        In the playground, open the **Operation Scripts** tab (next to **Variables** and **Headers**). It holds the **Pre-Flight Script**, which applies everywhere, along with the pre-operation and post-operation scripts for the active tab.
    </Step>
