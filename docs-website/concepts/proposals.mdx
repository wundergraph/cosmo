...
<Note>
  When adding a new subgraph, its labels must match the label matchers of the federated graph the proposal is for. A proposal with a new subgraph whose labels do not match is rejected, since the subgraph would not be part of that federated graph.
</Note>
...
Use the [`wgc proposal approve`](/cli/proposal/approve) command to approve an existing proposal. The command is non-interactive and can be used in CI with a `COSMO_API_KEY` that has write access to the federated graph:

```bash
npx wgc proposal approve <name> --federation-graph <federated-graph-name> [--namespace <namespace>]
```
