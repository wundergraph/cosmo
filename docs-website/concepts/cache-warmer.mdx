...
Size `timeout` based on the throughput the other two settings allow. The theoretical minimum runtime is `total items / items_per_second`. For example, warming 200 items with 1 worker at 2 items per second cannot finish in less than 100 seconds, and in practice takes longer because each item also has to be planned. A reasonable starting point is at least 2 times that minimum (best case).
