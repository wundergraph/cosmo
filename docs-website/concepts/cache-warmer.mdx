...
Three settings control how the router warms its cache at startup and how long startup will wait for warming to complete. Tune them to balance warm-up speed against system load and deploy time.
...
`items_per_second` is a ceiling, not a target. The router uses a leaky-bucket limiter that prevents workers from exceeding this rate, but actual throughput can be lower when planning is slow or there are fewer items than the limit allows. Set to `0` to disable the limiter.

Size `timeout` based on the throughput the other two settings allow. The theoretical minimum runtime is `total items / items_per_second`. For example, warming 200 items with 1 worker capped at 2 items per second cannot finish in less than 100 seconds, and in practice takes longer because each item also has to be planned.

If you are using the [slow plan cache](#slow-plan-cache) as the warmup source, to get an estimate of how many operations the router actually warms, inspect the cache warmer logs. The `Warmup completed`, `Warmup timeout`, and `Warmup error` messages all include a `processed_items` attribute. Use that number to size `timeout` for future restarts.

**Tuning guidance**

- For fastest warmup, raise `workers` and `items_per_second` together. Workers without throughput headroom will idle.
- For lowest startup load, lower `items_per_second` and keep `workers` modest.
- For predictable deploy time, keep `timeout` aligned with your deployment health-check window.
