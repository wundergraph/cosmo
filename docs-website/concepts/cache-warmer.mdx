...
Three settings shape warmup throughput: `workers`, `items_per_second`, and `item_delay`. The `timeout`, bounds how long startup will wait for warming to complete. Tune them to balance warm-up speed against system load and deploy time.
...
- For a hard floor on per-item spacing when some plans run slow, set `item_delay`. It pairs with `items_per_second` rather than replacing it.
