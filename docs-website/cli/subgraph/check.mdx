...
<Info>
   This command also works for feature subgraphs. Pass the feature subgraph name to validate a proposed feature subgraph schema against the feature flag compositions it belongs to. See [Schema Checks](/studio/schema-checks#feature-subgraphs-and-feature-flags). Composition errors, composition warnings, and composed breaking changes include a `FEATURE_FLAG` column identifying the feature flag each entry belongs to (empty for the base supergraph).
</Info>
