...
By default, only a contract of the base federated graph will be generated, which can be be disabled by `--disable-base-contract`.
In addition (or alternatively), contract can be generated for specific feature flag names if passed to `--contract-feature-flag-names`.
Only feature flags passed as an argument to this option will generate contracts.
Defining tags but disabling the base contract and not providing any feature flag names will produce an error.
