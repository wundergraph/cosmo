...
<Note>
  Note: The `--split-configs-enabled` flag requires Router version [0.315.0](https://github.com/wundergraph/cosmo/releases/tag/router%400.315.0) or later.
</Note>
...
* When using the `--split-configs-enabled` option, the `--out` is treated as a directory rather than a file.
