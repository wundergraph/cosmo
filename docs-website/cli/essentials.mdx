...
Assuming we have the following subgraphs:

1\. Products `--label team=A region=us-east-1`

2\. Employees `--label team=B region=us-east-1`

3\. Hobbies `--label team=A`

4\. Users `--label region=us-east-1`

Based on the matcher above, this setup will create a federated graph consisting of subgraphs *Products* and *Employees*. *Hobbies* is excluded because it has no `region` label, and *Users* is excluded because it has no `team` label.
