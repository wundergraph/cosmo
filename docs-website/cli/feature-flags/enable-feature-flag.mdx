...
<Info>
  Enabling a feature flag regenerates the [PQL manifest](/router/persisted-queries/persisted-operations#pql-manifest) for all affected federated graphs. Persisted operations that are only valid on this feature flag's schema are restored in the manifest.
</Info>
