...
Arguments can be defined on a root field, which can then be passed to event subjects or topics through an argument template. Argument templates are supported for NATS subjects and Kafka topics. With Kafka, the rendered topics must already exist, so the subscription filter is still useful when you want to split a stream without pre-creating many topics.

The argument template should follow the period delimiter of your subject or topic. The `args` refers to the field arguments, which has its own period delimiter, followed by a name of a field argument. This argument name must match exactly to a corresponding field argument name.
