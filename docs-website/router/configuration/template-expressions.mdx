...
- `subgraph.response.cache.status` (string): What the [response cache](/router/response-cache) did for the fetch: `hit`, `partial_hit` or `miss`. Empty when the response cache is not enabled.
...
Client trace values describe the individual subgraph fetch that is being logged or measured. They are populated in subgraph-scoped expressions such as subgraph access log fields and telemetry attribute expressions. In router-level access logs they are always zero.
...
- `subgraph.request.clientTrace.connAcquireDuration` (time.Duration): The duration it took to acquire the connection to the subgraph. This includes the dial time if a connection was not reused. If a connection was not successfully acquired, it will be 0. In case of retries, only the last attempt is recorded.
- `subgraph.request.clientTrace.dnsLookupDuration` (time.Duration): The duration of the DNS lookup for the subgraph request. It is 0 when a reused connection skips the lookup. In case of retries, the value of the last attempt that performed a lookup is recorded.
