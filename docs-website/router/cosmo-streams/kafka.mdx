...
<Note>
  Kafka topic arguments support the same argument template syntax as the other event providers, for example `employeeUpdated.{{ args.employeeID }}`.
  When you use topic templates with Kafka, you must create the rendered Kafka topics in your broker ahead of time. The Router does not create Kafka topics automatically.
</Note>
