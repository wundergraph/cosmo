...
The provider also accepts an optional `pool` block to tune the connection pool the router keeps to Redis. See [Redis Connection Pool](/router/configuration#redis-connection-pool).
