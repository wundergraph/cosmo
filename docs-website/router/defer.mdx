...
With the `@defer` directive, a client marks the fragments that may arrive later.
The router sends every available field in the initial response.
The router sends the remaining fields in later parts of the same multipart HTTP response.
...
The router disables `@defer` support by default.
...
An operation can disable all its `@defer` directives through `@defer(if: false)`.
The router executes such an operation as a regular operation.
The router answers it with a single JSON response, whatever the `Accept` header contains.
