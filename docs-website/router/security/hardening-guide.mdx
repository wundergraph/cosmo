...
You can apply different rate limits to specific keys by defining overrides under `simple_strategy`. Each override includes a `matching` field, a [Go regex](https://pkg.go.dev/regexp/syntax) tested against the resolved rate-limit key (after any `key_suffix_expression` is applied). The first matching override wins; unmatched keys fall back to the global defaults.
