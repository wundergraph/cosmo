...
{/* TODO: confirm the router release version that first ships this feature before publishing. */}

## Minimum requirements

| Package | Minimum version                                                               |
| ------- | ----------------------------------------------------------------------------- |
| router  | [0.330.0](https://github.com/wundergraph/cosmo/releases/tag/router%400.330.0) |
...
`disallow_inline_arguments` detects these arguments during operation normalization. Depending on the mode,
...
| `enabled-non-enforcing` | The router detects inline arguments and records them. The operation still executes. Use this to observe traffic before enforcing. |
| `enabled-enforcing`     | The router rejects any operation that uses an inline argument value. No subgraph requests are made.                              |

## Configuration

The feature is configured under `engine.disallow_inline_arguments`:
...
| ------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `mode`                          | `ENGINE_DISALLOW_INLINE_ARGUMENTS_MODE`                        | `off`                                                       | `off`, `enabled-non-enforcing`, or `enabled-enforcing`. See [Modes](#modes).                                                                     |
| `enforce_http_status_code`      | `ENGINE_DISALLOW_INLINE_ARGUMENTS_ENFORCE_HTTP_STATUS_CODE`    | `400`                                                       | HTTP status code returned when an operation is rejected in enforcing mode.                                                                       |
| `error_code`                    | `ENGINE_DISALLOW_INLINE_ARGUMENTS_ERROR_CODE`                  | `INLINE_ARGUMENT_VALUES_NOT_ALLOWED`                        | The `extensions.code` set on the rejection error.                                                                                               |
| `error_message`                 | `ENGINE_DISALLOW_INLINE_ARGUMENTS_ERROR_MESSAGE`              | `Inline argument values are not allowed. Use variables instead.` | The error message returned to the client on rejection.                                                                                          |
| `include_persisted_operations`  | `ENGINE_DISALLOW_INLINE_ARGUMENTS_INCLUDE_PERSISTED_OPERATIONS` | `false`                                                     | When `true`, the policy also applies to persisted operations. Persisted operations are exempt by default. See [Persisted operations](#persisted-operations). |
| `return_in_response_extensions` | `ENGINE_DISALLOW_INLINE_ARGUMENTS_RETURN_IN_RESPONSE_EXTENSIONS` | `false`                                                     | When `true`, detected inline arguments are returned to the client under `extensions.inlineArguments`. Applies only in non-enforcing mode. See [Response extensions](#response-extensions). |

These options are also listed in the [router configuration reference](/router/configuration#disallow-inline-arguments).
...
In `enabled-non-enforcing` mode the operation executes normally. The router surfaces the findings in two ways.
...
In `enabled-enforcing` mode the router rejects any operation that uses an inline argument value. The operation
...
Start in `enabled-non-enforcing` mode to understand your traffic without breaking clients.
...
migrate them to variables. Once the offending operations are gone, switch to `enabled-enforcing`.

```yaml
engine:
  disallow_inline_arguments:
    mode: enabled-enforcing
```
