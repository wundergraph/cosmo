...
A custom response writer that wraps a subscription response must implement `http.Flusher`.

To keep the SSE write timeout, the writer must also implement `SetWriteDeadline(time.Time) error` or `Unwrap() http.ResponseWriter`:
...
// Forward write deadlines to the wrapped writer.
...
// Or expose the wrapped writer. The router then finds SetWriteDeadline on it.
