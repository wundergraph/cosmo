...
* `router_engine_resolvers_max_concurrent`: The configured maximum number of concurrent GraphQL resolver slots, as set by `ENGINE_MAX_CONCURRENT_RESOLVERS`.

* `router_engine_resolvers_inflight`: The number of GraphQL resolver slots currently in use.

* `router_graphql_resolver_acquire_duration`: The time in ms spent waiting to acquire a resolver concurrency slot. A rising value indicates resolver pool saturation.
...
* `router_http_client_dns_lookup_duration`: The DNS lookup duration in ms for outgoing subgraph requests. Recorded as `0` when a cached or reused connection skips the lookup.

* `router_http_client_tcp_connect_duration`: The TCP connect (dial) duration in ms for outgoing subgraph requests. Recorded only when a new connection is established.

* `router_http_client_tls_handshake_duration`: The TLS handshake duration in ms for outgoing subgraph requests. Recorded only for new TLS connections.
