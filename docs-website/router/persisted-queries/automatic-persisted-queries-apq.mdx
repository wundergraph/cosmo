...
The connection pool can also be tuned explicitly with a `pool` block on the storage provider. See [Redis Connection Pool](/router/configuration#redis-connection-pool).
</Info>
<Note>
