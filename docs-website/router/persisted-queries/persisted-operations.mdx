...
A custom ID cannot be sent alongside a `query` body. A 64-character hexadecimal ID is treated as a custom ID if it differs from the published body’s SHA256. Requests containing both an ID and a `query` body still require the ID to match the body’s SHA256 exactly. Unknown IDs return `PersistedQueryNotFound`.
