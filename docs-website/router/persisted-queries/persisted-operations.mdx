...
When persisted operations are configured and APQ is disabled, published custom IDs are supported automatically with individual-operation storage or manifest lookup. With APQ enabled, IDs must be SHA256 hashes and retain the existing registration and lookup behavior.
...
Requests containing both a SHA256 ID and a `query` body remain supported when the ID matches the body’s SHA256 exactly. Mismatches are rejected. Custom IDs must be sent without a `query` body; this includes 64-character hexadecimal IDs that differ from the published body’s SHA256. Unknown IDs return `PersistedQueryNotFound`.
...
Enable `PERSISTED_OPERATIONS_MANIFEST_ENABLED=true` with APQ disabled to load all published operations into memory. Manifest warmup supports custom IDs. Custom-ID cache entries are scoped to the ID and the body’s SHA256, computed when the manifest loads. Unchanged operations remain cached across manifest updates. Each request checks the current manifest, so removed operations stop resolving and changed bodies cannot reuse stale cache entries.
