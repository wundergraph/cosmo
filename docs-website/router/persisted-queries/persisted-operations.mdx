...
Enabling custom IDs preserves support for requests containing both a SHA256 ID and a `query` body when the ID matches the body’s SHA256 exactly. Mismatches are rejected. Custom IDs must be sent without a `query` body; this includes 64-character hexadecimal IDs that differ from the published body’s SHA256. Unknown IDs return `PersistedQueryNotFound`.
...
Enable `PERSISTED_OPERATIONS_MANIFEST_ENABLED=true` alongside `PERSISTED_OPERATIONS_ALLOW_CUSTOM_IDS=true` to load all operations into memory. Manifest warmup supports custom IDs. Custom-ID cache entries are scoped to the ID and the body’s SHA256, computed when the manifest loads. Unchanged operations remain cached across manifest updates. Each request checks the current manifest, so removed operations stop resolving and changed bodies cannot reuse stale cache entries.
