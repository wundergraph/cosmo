...
`wgc operations push` preserves the IDs supplied in a manifest or query map. With APQ disabled, IDs can contain 1–250 ASCII letters, digits, underscores, or hyphens. With APQ enabled, IDs must be SHA256 hashes of the operation body. Plain GraphQL files always use the body’s SHA256 as the ID.

The following formats are detected automatically:
...
<Tabs>
  ...
  <Tab title="Relay">
    ...
    {
      "get_employees_v1": "query GetEmployees { employees { id } }",
      "update_employee_v1": "mutation UpdateEmployee($id: ID!) { updateEmployee(id: $id) { id } }"
    }
    ```
...
Send the published ID in `extensions.persistedQuery.sha256Hash`, despite the field name, with the `graphql-client-name: web` header:
...
Manifest-mode routers observe deletions after polling the updated manifest. Routers using individual-operation CDN lookup may retain cached bodies, so do not reuse deleted IDs while they may still be cached.
...
<Note>
  When the manifest is enabled, it is **authoritative** — the router does not fall back to fetching individual operations from the CDN. Unknown operation IDs are rejected immediately.
</Note>

With APQ disabled, manifest lookup supports both SHA256 and custom IDs. Unchanged operations remain cached across manifest updates. Each request checks the current manifest, so removed operations stop resolving and changed bodies cannot reuse stale cache entries.
...
4. **Block non-persisted operations** — Fully enforced blocking of non-persisted operations, clients are required to send a published operation ID instead of a query body.
