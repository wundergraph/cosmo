...
For an [incremental `@defer` query](/router/incremental-delivery), the same request extensions are forwarded to both the primary subgraph requests and subgraph requests started by deferred loaders.
