...
* `wg.response_cache.status`: Whether the fetch was answered by the [response cache](/router/response-cache): `hit`, `partial_hit` or `miss`. Only present while the response cache is enabled.
...
These metrics expose how the GraphQL engine's resolver concurrency pool is used. The router limits the number of resolvers that can run at the same time (see [`max_concurrent_resolvers`](/router/configuration#router-engine-configuration)). Use these metrics to detect when operations are queuing because the pool is saturated.
...
* `router.engine.resolvers.max_concurrent`: The configured maximum number of concurrent GraphQL resolver slots, as set by `ENGINE_MAX_CONCURRENT_RESOLVERS`.

* `router.engine.resolvers.inflight`: The number of GraphQL resolver slots currently in use. When this approaches `router.engine.resolvers.max_concurrent`, operations start waiting for a free slot.

* `router.graphql.resolver.acquire_duration`: Histogram (ms) of the time spent waiting to acquire a resolver concurrency slot. A rising value indicates resolver pool saturation.
...
* `router.http.client.dns_lookup_duration`: Histogram (ms) of the DNS lookup duration for outgoing subgraph requests. Recorded as `0` when a cached or reused connection skips the lookup.

* `router.http.client.tcp_connect_duration`: Histogram (ms) of the TCP connect (dial) duration for outgoing subgraph requests. Recorded only when a new connection is established.

* `router.http.client.tls_handshake_duration`: Histogram (ms) of the TLS handshake duration for outgoing subgraph requests. Recorded only for new TLS connections.
