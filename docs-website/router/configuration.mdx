...
### Disallow Inline Arguments

The configuration for [Disallow Inline Arguments](/router/security/disallow-inline-arguments). Detects, and
optionally rejects, operations that pass argument values inline instead of through variables.

| Environment Variable                                            | YAML                            | Required               | Description                                                                                                                                     | Default Value                                              |
|-----------------------------------------------------------------|---------------------------------|------------------------|------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------|
| ENGINE_DISALLOW_INLINE_ARGUMENTS_MODE                          | mode                            | <Icon icon="square" /> | `off` disables the feature; `enabled-non-enforcing` detects and records inline arguments while still executing; `enabled-enforcing` rejects operations that use inline argument values. | off                                                       |
| ENGINE_DISALLOW_INLINE_ARGUMENTS_ENFORCE_HTTP_STATUS_CODE       | enforce_http_status_code        | <Icon icon="square" /> | HTTP status code returned when an operation is rejected in enforcing mode.                                                                      | 400                                                       |
| ENGINE_DISALLOW_INLINE_ARGUMENTS_ERROR_CODE                     | error_code                      | <Icon icon="square" /> | The `extensions.code` set on the rejection error.                                                                                              | INLINE_ARGUMENT_VALUES_NOT_ALLOWED                        |
| ENGINE_DISALLOW_INLINE_ARGUMENTS_ERROR_MESSAGE                  | error_message                   | <Icon icon="square" /> | The error message returned to the client on rejection.                                                                                         | Inline argument values are not allowed. Use variables instead. |
| ENGINE_DISALLOW_INLINE_ARGUMENTS_INCLUDE_PERSISTED_OPERATIONS   | include_persisted_operations    | <Icon icon="square" /> | When true, the policy also applies to persisted operations. Persisted operations are exempt by default.                                        | false                                                     |
| ENGINE_DISALLOW_INLINE_ARGUMENTS_RETURN_IN_RESPONSE_EXTENSIONS  | return_in_response_extensions   | <Icon icon="square" /> | When true, detected inline arguments are returned to the client under `extensions.inlineArguments`. Applies only in non-enforcing mode.        | false                                                     |
