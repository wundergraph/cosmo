...
|                      | tls.enabled                       | <Icon icon="square" />                        | Enable TLS. Other `tls.*` fields are ignored unless this is `true`.                      | false         |
...
Set `tls.enabled: true` to turn on TLS. Other `tls.*` fields are ignored when `enabled` is `false` or omitted. The NATS provider supports four TLS modes:

| Mode | Config |
|---|---|
| No TLS | Omit the `tls` block, or set `enabled: false` |
| TLS with system trust store | `enabled: true` |
| TLS with skip verify | `enabled: true` + `insecure_skip_verify: true` |
| TLS with custom CA | `enabled: true` + `ca_file: /path/to/ca.crt` |
| Mutual TLS (mTLS) | `enabled: true` + `cert_file` + `key_file` (plus optional `ca_file`) |

<CodeGroup>
  ```yaml System trust store
  events:
    providers:
      nats:
        - id: my-nats
