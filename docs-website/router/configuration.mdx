...
| CORS_CUSTOM_SCHEMES    | custom_schemes    | <Icon icon="square" /> | Additional URL schemes permitted in `allow_origins`, including the `://` suffix.                                                               | []                                   |
...
Wildcard origins retain their existing validation and matching behavior. Patterns containing `*`, including `*.example.com`, bypass scheme validation and do not require `custom_schemes`.

To allow a literal origin with a custom URL scheme, add the scheme to `custom_schemes` and the origin to `allow_origins`. Adding a scheme does not allow every origin using that scheme. For example:
