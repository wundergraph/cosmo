...
| CORS_CUSTOM_SCHEMAS    | custom_schemas    | <Icon icon="square" /> | Additional URL schemes permitted in `allow_origins`, including the `://` suffix.                                                               | []                                   |
...
Wildcard origins retain their existing validation and matching behavior. Patterns containing `*`, including `*.example.com`, bypass scheme validation and do not require `custom_schemas`.
...
To allow a literal origin with a custom URL scheme, add the scheme to `custom_schemas` and the origin to `allow_origins`. Adding a scheme does not allow every origin using that scheme. For example:
...
Patterns in `match_origins` can also match custom URL schemes directly. `custom_schemas` applies only to entries in `allow_origins`.
