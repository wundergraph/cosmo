...
| on_error | <Icon icon="square" /> | Handle invalid JWT credentials with `reject` (generic 401 response) or `continue` (ignore the credential). Required authentication and field authorization still apply. See [JWT validation error handling](/router/authentication-and-authorization#configuring-jwt-validation-error-handling). | reject |
