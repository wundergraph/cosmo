...
Regex matching is case-insensitive by default and uses the original `Origin` header value without URL normalization. Prefix a pattern with `(?-i)` for case-sensitive matching, for example `(?-i)^https://app\.example\.com$`. Use `(?-i:...)` to make only a group case-sensitive.
