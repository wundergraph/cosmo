...
Origins in `allow_origins` can use any URL scheme. List the full origin, such as `custom://localhost`, to allow that origin. Wildcard patterns are supported. The default `["*"]` allows all origins, regardless of scheme.
