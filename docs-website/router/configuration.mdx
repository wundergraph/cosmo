...
| Environment Variable                               | YAML                                               | Required                | Description                                                                                              | Default Value |
| -------------------------------------------------- | -------------------------------------------------- | ----------------------- | -------------------------------------------------------------------------------------------------------- | ------------- |
| TLS_CLIENT_GRPC_ALL_ENABLED                        | tls.client_grpc.all.enabled                        | <Icon icon="check-square" /> | Enable TLS for all gRPC subgraph connections.                                                       |               |
...
| METRICS_OTLP_EXEMPLAR_FILTER       | exemplar_filter       | <Icon icon="square" />                        | Controls which measurements are eligible to record exemplars. One of: `always_off`, `trace_based`, `always_on`. See [Exemplars](/router/metrics-and-monitoring/exemplars). | always_off    |
...
| PROMETHEUS_EXEMPLAR_FILTER       | exemplar_filter       | <Icon icon="square" />                        | Controls which measurements are eligible to record exemplars. One of: `always_off`, `trace_based`, `always_on`. See [Exemplars](/router/metrics-and-monitoring/exemplars). | always_off       |
...
Continuously profile the router with [Grafana Pyroscope](https://grafana.com/oss/pyroscope/). When enabled, the router periodically collects profiles (CPU, memory, and more) and uploads them to a Pyroscope server, so you can analyze the router's performance over time in Grafana. This works against a self-hosted Pyroscope instance or [Grafana Cloud Profiles](https://grafana.com/products/cloud/profiles/). See the [Profiling guide](/router/profiling#continuous-profiling-with-grafana-pyroscope) for a walkthrough.

| Environment Variable             | YAML                    | Required                                    | Description                                                                                                                                                                                                                                    | Default Value                                              |
| -------------------------------- | ----------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| PYROSCOPE_ENABLED                | enabled                 | <Icon icon="square" />                      | Enable continuous profiling. When enabled, `server_address` must be set.                                                                                                                                                                       | false                                                      |
| PYROSCOPE_SERVER_ADDRESS         | server_address          | <Icon icon="square-check" iconType="solid" /> (when enabled) | Address of the Pyroscope server the profiles are sent to, e.g. your own Pyroscope or a Grafana Cloud instance.                                                                                                                                  |                                                            |
| PYROSCOPE_APPLICATION_NAME       | application_name        | <Icon icon="square" />                      | Application name the profiles are stored under.                                                                                                                                                                                                | wundergraph.cosmo.router                                   |
| PYROSCOPE_BASIC_AUTH_USERNAME    | basic_auth.username     | <Icon icon="square" />                      | Username for basic authentication. For Grafana Cloud, this is the stack user ID.                                                                                                                                                               |                                                            |
| PYROSCOPE_BASIC_AUTH_PASSWORD    | basic_auth.password     | <Icon icon="square" />                      | Password for basic authentication. For Grafana Cloud, this is an access token with profile write permissions.                                                                                                                                  |                                                            |
| PYROSCOPE_HEADERS                | headers                 | <Icon icon="square" />                      | Map of headers to send with each profile upload request, e.g. to authenticate with a bearer token instead of basic auth. As an environment variable, use the format `key1:value1,key2:value2`.                                                  |                                                            |
| PYROSCOPE_TAGS                   | tags                    | <Icon icon="square" />                      | Map of static tags attached to all profiles. The router always attaches the hostname and router version as tags. As an environment variable, use the format `key1:value1,key2:value2`.                                                          |                                                            |
| PYROSCOPE_UPLOAD_RATE            | upload_rate             | <Icon icon="square" />                      | Interval at which profiles are uploaded to the Pyroscope server (e.g. `15s`, `1m`).                                                                                                                                                            | 15s                                                        |
| PYROSCOPE_PROFILE_TYPES          | profile_types           | <Icon icon="square" />                      | Profile types to collect. Valid values: `cpu`, `alloc_objects`, `alloc_space`, `inuse_objects`, `inuse_space`, `goroutines`, `mutex_count`, `mutex_duration`, `block_count`, `block_duration`, `goroutine_leak`. Unrecognized values are ignored. | cpu, alloc_objects, alloc_space, inuse_objects, inuse_space |
| PYROSCOPE_DISABLE_GC_RUNS        | disable_gc_runs         | <Icon icon="square" />                      | Disable the automatic `runtime.GC` run before each heap profile upload. Enabling this avoids extra GC pauses on large-heap or latency-sensitive routers, at the cost of less accurate memory profiles.                                          | false                                                      |
| PYROSCOPE_MUTEX_PROFILE_FRACTION | mutex_profile_fraction  | <Icon icon="square" />                      | Fraction of mutex contention events reported in the mutex profile; on average 1/n events are reported. Set to `0` to disable mutex profiling.                                                                                                   | 5                                                          |
| PYROSCOPE_BLOCK_PROFILE_RATE     | block_profile_rate      | <Icon icon="square" />                      | Fraction of goroutine blocking events reported in the block profile; on average 1/n events are reported. Set to `0` to disable block profiling.                                                                                                 | 5                                                          |
...
pyroscope:
  ...
  application_name: "wundergraph.cosmo.router"
  # Optional: authenticate against Grafana Cloud
  basic_auth:
...
| CORS_MATCH_ORIGINS     | match_origins     | <Icon icon="square" /> | Go regular expressions matched against the lowercased origin in addition to `allow_origins`. Patterns are compiled as written and matching is unanchored. Separate patterns with semicolons in the environment variable. | []                            |
...
Origins in `allow_origins` can use any URL scheme. Wildcard patterns are supported. The default `["*"]` allows all origins, regardless of scheme.

### Regular expression origins

Use `match_origins` to allow origins using [Go regular expressions](https://pkg.go.dev/regexp/syntax). When CORS is enabled, invalid patterns prevent router startup and the error identifies the pattern. Lookarounds and backreferences are not supported.

The `allow_origins` and `match_origins` lists are additive. Set `allow_origins: []` for regex-only matching. Its default `["*"]` allows every origin, even when `match_origins` is set.

Patterns match any substring of the origin unless you add anchors. Use `^` and `$` to match the entire origin. For example, `^https://([a-z0-9-]+\.)*example\.com$` allows `https://example.com` and `https://app.example.com`, but rejects `https://app.example.com.evil.com`. Without the anchors, that pattern also matches `https://app.example.com.evil.com`. Include any allowed port explicitly in the pattern.

The router lowercases the `Origin` header value before literal, wildcard, and regex matching. This normalizes scheme and host casing, as in serialized browser origins. Ports are preserved. Regex patterns are compiled exactly as configured, with Go's default case-sensitive matching. Write scheme and host literals in lowercase, or add `(?i)` explicitly. For example, `^https://app\.example\.com$` matches `HTTPS://APP.EXAMPLE.COM`, but `^HTTPS://APP\.EXAMPLE\.COM$` does not. Regex escapes such as `\D` retain their original meaning.
...
cors:
  ...
  match_origins:
    ...
    - '^https://(.+\.)?aol\.(de|ca|co\.uk|com)$'
```

Use single-quoted YAML strings to preserve regex backslashes. Separate patterns in `CORS_MATCH_ORIGINS` with semicolons (`;`) so commas in repetition counts such as `{1,3}` are preserved. For regex-only matching with this environment variable, set `allow_origins: []` in YAML. Leaving `CORS_ALLOW_ORIGINS` empty does not clear its default.

Requests and preflights allowed by a regex echo the original `Origin` header in `Access-Control-Allow-Origin`, preserving its casing. Cross-origin requests that match neither list receive `403 Forbidden`.

### Custom URL schemes

List the full origin in `allow_origins`, such as `custom://localhost`, to allow that origin.
...
Patterns in `match_origins` can also match custom URL schemes, for example `^custom://localhost$`.
...
|                      | from_file                  | <Icon icon="square" />                        | If `op` is `set`, sources the header value from a file's contents. The file is loaded into memory and refreshed on a polling interval, so request-time reads do not touch disk. Request rules only. |               |
|                      | from_file.path             | <Icon icon="square-check" iconType="solid" /> | Absolute path to the file whose contents become the header value.                                                                                                                 |               |
|                      | from_file.refresh_interval | <Icon icon="square-check" iconType="solid" /> | Determines how often the file is checked for changes and re-read into memory. The minimum value is `100ms`.                                                                       | `1m`          |
...
| STORAGE_PROVIDER_REDIS_\{n\}_URLS            | urls            | <Icon icon="square-check" iconType="solid" /> | List of Redis URLs to connect to. If cluster_enabled is true, these are the seeds to discover the cluster. Connection and pool settings go in the URL query string, see [Redis Connection URLs](/router/configuration#redis-connection-urls) |               |
...
Everywhere the router connects to Redis it takes a connection URL: `rate_limit.storage`, each entry under `storage_providers.redis`, and each entry under `events.providers.redis`. There is no separate block for connection or pool settings. The Redis client parses them from the URL query string, so you tune them there.

```yaml config.yaml
rate_limit:
  storage:
    urls:
      - "redis://user:password@localhost:6379/0?pool_size=20&min_idle_conns=2&pool_timeout=3s"
```

Use the `rediss://` scheme instead of `redis://` to connect over TLS. There is no query parameter for this.

<Note>
  A parameter the client does not recognise makes the router fail at startup with `redis: unexpected option: <name>`, so a typo in a pool setting cannot be silently ignored. This also means options from other Redis clients (for example `ssl=true` or `connectTimeout`) are not accepted.
</Note>

### Valkey

[Valkey](https://valkey.io) works as a drop-in replacement for Redis in every place the router accepts a Redis URL: rate limiting, automatic persisted queries, the response cache, and the Redis event provider, in both standalone and cluster mode. Keep the `redis://` and `rediss://` schemes; there is no `valkey://` scheme.

### Connection pool options

| Parameter          | Description                                                                                                                                          | Default Value       |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| pool_size          | Maximum number of socket connections the pool keeps open. Commands arriving while every connection is busy wait up to `pool_timeout` for one to free up. | 10 * number of CPUs |
| min_idle_conns     | Number of idle connections kept ready, so a burst of traffic does not pay the cost of establishing connections on the request path.                    | 0                   |
| max_idle_conns     | Upper bound on idle connections held by the pool.                                                                                                     | no limit            |
| max_active_conns   | Hard cap on connections, including those handed out to in-flight commands. Unlike `pool_size` this cap fails fast. See the warning below.             | no limit            |
| pool_timeout       | How long a command waits for a free connection before failing, e.g. `3s`.                                                                            | read timeout + 1s   |
| conn_max_idle_time | Retire a connection idle for longer than this, e.g. `5m`. A negative value disables the check.                                                        | 30m                 |
| conn_max_lifetime  | Retire a connection at this age, measured from when it was created, e.g. `30m`. Useful when a proxy in front of Redis needs connections recycled to rebalance. | disabled            |
| pool_fifo          | Use FIFO instead of LIFO when handing out pooled connections. LIFO keeps the pool warm; FIFO recycles connections more evenly.                        | false               |

### Connection and retry options

| Parameter          | Description                                                                       | Default Value |
| ------------------ | --------------------------------------------------------------------------------- | ------------- |
| dial_timeout       | Timeout for establishing a new connection, e.g. `5s`.                             | 5s            |
| read_timeout       | Socket read timeout, e.g. `3s`. Also the basis for the default `pool_timeout`.     | 3s            |
| write_timeout      | Socket write timeout, e.g. `3s`.                                                  | read timeout  |
| max_retries        | Number of retries before giving up on a command. `-1` disables retries.           | 3             |
| min_retry_backoff  | Minimum backoff between retries, e.g. `8ms`. `-1` disables backoff.               | 8ms           |
| max_retry_backoff  | Maximum backoff between retries, e.g. `512ms`. `-1` disables backoff.             | 512ms         |
| client_name        | Name sent to Redis with `CLIENT SETNAME`, which makes the router's connections identifiable in `CLIENT LIST`. |               |
| protocol           | RESP protocol version, `2` or `3`.                                                | 3             |

### Single node only

| Parameter | Description                                                                        | Default Value |
| --------- | ---------------------------------------------------------------------------------- | ------------- |
| db        | Database index to select. Also settable as the URL path, e.g. `redis://host:6379/2`. | 0             |

### Cluster only

These apply when `cluster_enabled: true`.
...
<Warning>
  ...
  ```yaml config.yaml
  urls:
    - "redis://localhost:7001?pool_size=20&max_redirects=5" # options read from here
    - "redis://localhost:7002"                              # host only
    - "redis://localhost:7003"                              # host only
  ```
</Warning>

<Warning>
  `pool_size` and `max_active_conns` are enforced differently. `pool_size` makes commands **wait** for a free connection, up to `pool_timeout`. `max_active_conns` **fails immediately** with `connection pool exhausted` once the cap is reached.
...
The default of 10 connections per CPU is generous. A pooled connection is reused rather than reconnected, so even a small pool sustains a high command rate. Benchmarking GET throughput against a local Redis with far more concurrent callers than connections showed steeply diminishing returns: 1 connection took ~83µs per command, 4 took ~35µs, and 16 took ~19µs, after which the curve flattens. The default of 140 connections on a 14-core machine was only about 15% faster than 16 connections, for nearly nine times as many open connections, and 128 connections were no faster than the default.

So if you run many router replicas against one Redis, setting `pool_size` explicitly is usually worth it: it cuts the connection count on the Redis side, which matters against its `maxclients` limit, for a modest throughput trade-off. Watch for `pool_timeout` errors after lowering it. Those mean commands are queueing longer than the timeout allows and the pool is now too small.
...
When APQ is disabled, configured persisted-operation storage accepts published IDs of 1–250 ASCII letters, digits, underscores, or hyphens, including SHA256 IDs. With APQ enabled, IDs must be SHA256 hashes. Requests containing both an ID and a body require the ID to match the body’s SHA256 exactly.
...
| EXECUTION_CONFIG_MANIFEST_PATH                       | manifest.path                                 | <Icon icon="square-check" iconType="solid" /> | The path to the manifest directory.                                                                                                                                                                                                          |               |
| EXECUTION_CONFIG_MANIFEST_SKIP_MISSING_FEATURE_FLAGS | manifest.skip_missing_feature_flags           | <Icon icon="square" />                        | Skip feature flags listed in `mapper.json` whose execution config file cannot be loaded, instead of aborting the load.                                                                                                                       | false         |
| EXECUTION_CONFIG_MANIFEST_IGNORED_FEATURE_FLAGS      | manifest.ignored_feature_flags                | <Icon icon="square" />                        | Feature flag names to skip entirely when loading the manifest. Listed flags are not loaded even when referenced by `mapper.json`.                                                                                                            |               |
| EXECUTION_CONFIG_MANIFEST_WATCH                      | manifest.watch                                | <Icon icon="square" />                        | Enable the watch mode. When enabled, the router stats `mapper.json` for changes and reloads the assembled execution config without downtime.                                                                                                 | false         |
| EXECUTION_CONFIG_MANIFEST_WATCH_INTERVAL             | manifest.watch_interval                       | <Icon icon="square" />                        | The interval at which `mapper.json` is checked for changes. Only applied when `manifest.watch` is enabled.                                                                                                                                   | 1s            |
...
|                      | default_subprotocol              | <Icon icon="square" />                        | The subprotocol to use when a client sends no `Sec-WebSocket-Protocol` header. One of `graphql-transport-ws` or `graphql-ws`. When empty, the router closes such connections. |               |
...
websocket:
  ...
  # used when a client sends no Sec-WebSocket-Protocol header
  default_subprotocol: graphql-transport-ws
...
| on_error | <Icon icon="square" /> | Handle invalid JWT credentials with `reject` (HTTP 401, or an error message and connection closure for WebSocket initial-payload authentication) or `continue` (ignore the credential). Required authentication and field authorization still apply. See [JWT validation error handling](/router/authentication-and-authorization#configuring-jwt-validation-error-handling). | reject |
...
| ENABLE_PRE_FETCH_FIELD_AUTHORIZATION | enable_pre_fetch_field_authorization | <Icon icon="square" /> | If enabled, fields protected by [@authenticated](/federation/directives/authenticated) or [@requiresScopes](/federation/directives/requiresscopes) are authorized in a single batch before any subgraph fetch runs, instead of filtering them from the response after the fetch. This avoids fetching data the client is not authorized to see. | false         |
...
authorization:
  ...
  enable_pre_fetch_field_authorization: false
```
...
| Environment Variable              | YAML                       | Required               | Description                                                                                                                                                                                                                                                                                                                       | Default Value |
| --------------------------------- | -------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| EVENTS_SKIP_UNAVAILABLE_PROVIDERS | skip_unavailable_providers | <Icon icon="square" /> | Start the router even when an event provider referenced by the execution config is unavailable: either not defined under `providers`, or defined but unreachable at startup (e.g. the broker is down). The router logs a warn and disables only the affected fields instead of failing to start. A provider that is unreachable reconnects in the background and its fields recover without a restart once the broker is back. Enabled by default. | true         |
...
|                      | tls.insecure_skip_ca_verification | <Icon icon="square" />                        | Skip server certificate verification. Not recommended for production.                    | false         |
|                      | tls.ca_file                       | <Icon icon="square" />                        | Path to a custom CA certificate file (PEM) used to verify the server certificate. Defaults to the host's system trust store. |               |
|                      | tls.cert_file                     | <Icon icon="square" />                        | Path to the client certificate file (PEM) for mTLS. Must be set together with `key_file`.             |               |
|                      | tls.key_file                      | <Icon icon="square" />                        | Path to the client private key file (PEM) for mTLS. Must be set together with `cert_file`.            |               |
...
|                      | urls |          | A list of redis instances URLS, e.g: `redis://localhost:6379/2`. Connection and pool settings go in the URL query string, see [Redis Connection URLs](/router/configuration#redis-connection-urls) |               |
|                      | cluster_enabled |          | If enabled, the Redis Cluster client is used and the URLs are treated as cluster seeds.   | false         |
...
|                      | handlers.before_events_dispatch.handler_timeout       | <Icon icon="square" /> | The maximum time to wait for all BeforeEventsDispatch handlers to complete per event-batch. | 5s          |
...
| ENGINE_SSE_SERVER_WRITE_TIMEOUT                               | sse_server_write_timeout                               | <Icon icon="square" /> | Write timeout for SSE responses from the router to clients. If a write takes longer, the router closes the subscription. Set `0s` to disable.                                                                                                                                                                                  | 10s           |
...
| ENGINE_ENABLE_MULTI_FETCH                                     | enable_multi_fetch                                     | <Icon icon="square" /> | Merges entity fetches that target the same subgraph and execute at the same point in the query plan into a single request with aliased `_entities` fields. This reduces the number of requests sent to subgraphs.                                                                                                                           | true          |
| ENGINE_ENABLE_SCHEDULE_FETCHES                                | enable_schedule_fetches                                | <Icon icon="square" /> | Replaces the wave-based fetch execution with a dependency-aware fetch scheduler. Independent fetch chains progress as soon as their own dependencies complete, instead of waiting for the slowest fetch in each wave.                                                                                                                       | true          |
...
| RATE_LIMIT_EXCLUDE_SUBSCRIPTIONS                                                         | exclude_subscriptions | <Icon icon="square" />                        | Exclude subscription operations from rate limiting. Queries and mutations remain rate limited, including those sent over subscription transports like WebSockets.                                                                                                                     | false         |
...
| Environment Variable             | YAML            | Required                                      | Description                                         | Default Value            |
| -------------------------------- | --------------- | --------------------------------------------- | --------------------------------------------------- | ------------------------ |
| RATE_LIMIT_REDIS_URLS            | urls            | <Icon icon="square-check" iconType="solid" /> | List of the connection URL(s). Connection and pool settings go in the URL query string, see [Redis Connection URLs](/router/configuration#redis-connection-urls) | [redis://localhost:6379] |
...
<Warning>
  ...
  If subscriptions are not excluded, every subgraph request done while the subscription is active will be subject to the rate limiting.
</Warning>
...
| Environment Variable                                     | YAML                          | Required               | Description                                                                                                                                                                | Default Value |
|----------------------------------------------------------|-------------------------------|------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------|
| SUBGRAPH_EXTENSION_PROPAGATION_ENABLED                   | enabled                       | <Icon icon="square" /> | Enable forwarding of subgraph response extensions to the client.                                                                                                           | false         |
| SUBGRAPH_EXTENSION_PROPAGATION_ALLOWED_EXTENSION_FIELDS  | allowed_extension_fields      | <Icon icon="square" /> | Extension fields that are allowed to be propagated from subgraphs to the client. Any field not in this list is dropped. If empty, all extension fields are forwarded.      | []            |
| SUBGRAPH_EXTENSION_PROPAGATION_ALGORITHM                 | algorithm                     | <Icon icon="square" /> | Conflict resolution when multiple subgraphs return the same extension field. `first_write` keeps the value from the first subgraph that wrote it; `last_write` keeps the last. | first_write   |
...
| SECURITY_COMPLEXITY_IGNORE_INTROSPECTION          | complexity_limits.ignore_introspection   | <Icon icon="square" /> | When true, complexity validation is skipped for all introspection queries.                                                        | false         |
...
security:
  ...
  complexity_limits:
    ...
    ignore_introspection: false # when true, complexity validation is skipped for all introspection queries
...
All four limits enforce their configured value, including 0, when `enabled: true` in `enforce` mode. Set `enabled: false` to disable a limit.

For `root_field_aliases`, a limit of 0 disallows root field aliases when enabled in `enforce` mode. Queries without root field aliases remain allowed. Nested field aliases are unaffected.
...
|                          | limit                       | <Icon icon="square" /> | The maximum allowed value, including 0. Set `enabled: false` to disable the limit.                                                                                                     | 0             |
...
#### Migrating from zero as a disabled limit

Previously, a limit of 0 disabled enforcement even with `enabled: true`. Enabled zero limits now reject operations whose measured value exceeds 0. Omitting `limit` has the same effect because its default is 0.

This affects `security.complexity_limits.depth`, `total_fields`, `root_fields`, and `root_field_aliases` in `enforce` mode. Omitting `mode` also enforces limits. The deprecated `security.depth_limit` is affected when it supplies the depth limit. An explicit `security.complexity_limits.depth` takes precedence over that deprecated setting.

Before upgrading the Router, set `enabled: false` on each limit that used zero to disable enforcement. This configuration works with both the previous and new behavior. For example:

```yaml
# Before: zero implicitly disabled this limit.
security:
  complexity_limits:
    depth:
      enabled: true
...
```yaml
# After: explicitly disable this limit before upgrading.
security:
  complexity_limits:
    depth:
      enabled: false
...
Apply the same change to the other affected limits, including `security.depth_limit` if it is still used. To enforce a limit instead, set an appropriate positive value. Keep `root_field_aliases.enabled: true` with `limit: 0` only when root aliases should be rejected.

Existing positive limits, disabled limits, and `measure` mode retain their behavior. Persisted operations remain exempt from a limit when its `ignore_persisted_operations` setting is true. No Control Plane, composition, or execution-configuration version change is required for this migration.
...
### Apollo Router Compatibility Skip Null Variables Error

Skips the validation error raised when `null` is supplied for a variable of a non-nullable type, matching Apollo Router behavior. Variables that are missing entirely still produce an error.

| Environment Variable                                          | YAML                      | Required               | Description                                                       | Default Value |
| ------------------------------------------------------------- | ------------------------- | ---------------------- | ----------------------------------------------------------------- | ------------- |
| APOLLO_ROUTER_COMPATIBILITY_SKIP_NULL_VARIABLES_ERROR_ENABLED | skip_null_variables_error | <Icon icon="square" /> | Skips the error when a null variable is supplied for a non-nullable type. | false         |
