...
| on_error | <Icon icon="square" /> | Handle request-time authentication errors with `reject` (generic 401 response) or `continue` (ignore invalid credentials and proceed unauthenticated, subject to existing authorization checks). Original headers remain available to configured subgraph forwarding rules. Includes credential extraction, JWT validation, and custom authenticator errors. Using `continue` requires credential validation before protected work. See [configuring JWT validation error handling](/router/authentication-and-authorization#configuring-jwt-validation-error-handling). | reject |
