...
| on_error | <Icon icon="square" /> | Handle request-time authentication errors with `reject` (generic 401 response) or `continue` (proceed unauthenticated). Required authentication and field authorization still apply. See [JWT validation error handling](/router/authentication-and-authorization#configuring-jwt-validation-error-handling). | reject |
