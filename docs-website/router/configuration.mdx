...
| CORS_MATCH_ORIGINS     | match_origins     | <Icon icon="square" /> | Go regular expressions matched against the entire origin, in addition to `allow_origins`. Separate patterns with semicolons in the environment variable. | []                            |
...
Each pattern must match the entire origin, including its scheme and optional port. Anchors such as `^` and `$` are optional. For example, `https://([a-z0-9-]+\.)*example\.com` allows `https://example.com` and `https://app.example.com`, but rejects `https://app.example.com.evil.com`. Matching is case-sensitive unless the pattern uses `(?i)`.
...
Requests allowed by a regex receive their exact `Origin` in `Access-Control-Allow-Origin`, including preflight responses. This supports credentialed requests when `allow_credentials` is enabled. Cross-origin requests that match neither list receive `403 Forbidden`.
...
Patterns in `match_origins` can also match custom URL schemes directly. `custom_schemes` applies only to entries in `allow_origins`.
