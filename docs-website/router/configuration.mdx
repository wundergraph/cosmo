...
See [Outbound mTLS to Subgraphs](/router/security/tls#router-mtls) for details.
...
For gRPC subgraphs there are dedicated environment variables.
...
tls:
  client_grpc:
    all: # global tls configuration affecting all gRPC subgraphs
      cert_file: /path/to/client.crt
      key_file: /path/to/client.key
      ca_file: /path/to/ca.crt
    subgraphs: # additional overwrites for specific gRPC subgraphs
      projects:
        cert_file: /path/to/products-client.crt
        key_file: /path/to/products-client.key
```
