...
| WEBSOCKETS_DEFAULT_SUBPROTOCOL | default_subprotocol    | <Icon icon="square" />                        | The subprotocol to use when a client sends no `Sec-WebSocket-Protocol` header. One of `graphql-transport-ws` or `graphql-ws`. When empty, the router closes such connections. |               |
