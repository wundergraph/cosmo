...
| CORS_MATCH_ORIGINS     | match_origins     | <Icon icon="square" /> | Go regular expressions matched against the origin in addition to `allow_origins`. Matching is unanchored and case-insensitive by default. Separate patterns with semicolons in the environment variable. | []                            |
...
Use `match_origins` to allow origins using [Go regular expressions](https://pkg.go.dev/regexp/syntax). The router compiles patterns at startup and rejects invalid patterns with an error naming the pattern. Matching uses Go's linear-time regex engine. Lookarounds and backreferences are not supported.

Patterns match any substring of the origin unless you add anchors. Use `^` and `$` to match the entire origin, including its scheme and optional port. For example, `^https://([a-z0-9-]+\.)*example\.com$` allows `https://example.com` and `https://app.example.com`, but rejects `https://app.example.com.evil.com`. Without the anchors, that pattern also matches `https://app.example.com.evil.com`.

Matching is case-insensitive by default. Prefix a pattern with `(?-i)` for case-sensitive matching, for example `(?-i)^https://app\.example\.com$`. Use `(?-i:...)` to make only a group case-sensitive.

An origin is allowed if it matches either `allow_origins` or `match_origins`. The existing `*` wildcard syntax in `allow_origins` is unchanged. Set `allow_origins: []` for regex-only matching. Leaving `allow_origins` at its default `["*"]` allows all origins regardless of `match_origins`.
...
Use single-quoted YAML strings to preserve regex backslashes. `CORS_MATCH_ORIGINS` uses semicolons to separate patterns so commas in repetition counts such as `{1,3}` are preserved. For regex-only matching with this environment variable, set `allow_origins: []` in YAML. An empty `CORS_ALLOW_ORIGINS` environment variable uses the default `*`.

Requests allowed by a regex receive their exact `Origin` in `Access-Control-Allow-Origin`, including preflight responses. Cross-origin requests that match neither list receive `403 Forbidden`.
...
Patterns in `match_origins` can also match custom URL schemes directly.
