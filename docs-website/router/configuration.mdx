...
The router forwards subgraph response `extensions` to the client. An allow list restricts which root extension fields are propagated, and an algorithm selects a winner when multiple subgraphs return the same root field.
