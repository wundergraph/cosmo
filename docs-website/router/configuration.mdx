...
|                                         | pool            | <Icon icon="square" />                        | Connection pool tuning. See [Redis Connection Pool](/router/configuration#redis-connection-pool)          |               |
...
|                      | pool |          | Connection pool tuning. See [Redis Connection Pool](/router/configuration#redis-connection-pool) |               |
...
Every place the router connects to Redis accepts an optional `pool` block that tunes the connection pool: `rate_limit.storage`, each entry under `storage_providers.redis`, and each entry under `events.providers.redis`.

Each property is optional. Anything you leave unset keeps the value taken from the connection URL query parameters (for example `redis://localhost:6379?pool_size=50`) and otherwise falls back to the go-redis default, so a `pool` block only overrides what it explicitly names.

<Note>
  For a Redis Cluster these limits apply **per node**, not to the cluster as a whole. A `size` of 20 against a six-node cluster permits up to 20 connections to each node.
</Note>

| YAML                 | Required               | Description                                                                                                                                                                             | Default Value       |
| -------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| size                 | <Icon icon="square" /> | Maximum number of socket connections the pool keeps open. Commands that arrive while every connection is busy wait up to `timeout` for one to free up.                                   | 10 * number of CPUs |
| min_idle_conns       | <Icon icon="square" /> | Number of idle connections kept ready, so a burst of traffic does not pay the cost of establishing connections on the request path.                                                      | 0                   |
| max_idle_conns       | <Icon icon="square" /> | Upper bound on idle connections held by the pool.                                                                                                                                       | no limit            |
| max_active_conns     | <Icon icon="square" /> | Hard cap on connections, including those handed out to in-flight commands. Unlike `size` this cap fails fast: see the warning below.                                                     | no limit            |
| timeout              | <Icon icon="square" /> | How long a command waits for a free connection once the pool is saturated before failing, e.g. `3s`.                                                                                     | read timeout + 1s   |
| conn_max_idle_time   | <Icon icon="square" /> | Retire a connection that has been idle for longer than this, e.g. `5m`. A negative value disables the check.                                                                             | 30m                 |
| conn_max_lifetime    | <Icon icon="square" /> | Retire a connection once it reaches this age, measured from when it was created, e.g. `30m`. Useful when a proxy or load balancer in front of Redis needs connections recycled to rebalance. | disabled            |

<Warning>
  `size` and `max_active_conns` are enforced differently. `size` makes commands **wait** for a free connection, up to `timeout`. `max_active_conns` **fails immediately** with `connection pool exhausted` once the cap is reached.

  Setting `max_active_conns` lower than `size` therefore admits more concurrent commands than there are connections available for, and the excess fails instead of queueing. Keep `max_active_conns` at or above `size`, or leave it unset. The router logs a warning at startup if you configure this combination.
</Warning>

The default `size` of 10 per CPU is generous. If you are running many router replicas against one Redis, lowering it reduces the connection count on the Redis side, usually with no loss in throughput — a single connection is enough to serve thousands of commands per second, since the pool reuses it rather than reconnecting.

Environment variables are available for the rate limit storage and for storage providers:

| Environment Variable                              | YAML                          |
| ------------------------------------------------- | ----------------------------- |
| RATE_LIMIT_REDIS_POOL_SIZE                        | rate_limit.storage.pool.size  |
| RATE_LIMIT_REDIS_POOL_MIN_IDLE_CONNS              | ...pool.min_idle_conns        |
| RATE_LIMIT_REDIS_POOL_MAX_IDLE_CONNS              | ...pool.max_idle_conns        |
| RATE_LIMIT_REDIS_POOL_MAX_ACTIVE_CONNS            | ...pool.max_active_conns      |
| RATE_LIMIT_REDIS_POOL_TIMEOUT                     | ...pool.timeout               |
| RATE_LIMIT_REDIS_POOL_CONN_MAX_IDLE_TIME          | ...pool.conn_max_idle_time    |
| RATE_LIMIT_REDIS_POOL_CONN_MAX_LIFETIME           | ...pool.conn_max_lifetime     |
| STORAGE_PROVIDER_REDIS_\{n\}_POOL_SIZE              | storage_providers.redis[n].pool.size |
| STORAGE_PROVIDER_REDIS_\{n\}_POOL_MIN_IDLE_CONNS    | ...pool.min_idle_conns        |
| STORAGE_PROVIDER_REDIS_\{n\}_POOL_MAX_IDLE_CONNS    | ...pool.max_idle_conns        |
| STORAGE_PROVIDER_REDIS_\{n\}_POOL_MAX_ACTIVE_CONNS  | ...pool.max_active_conns      |
| STORAGE_PROVIDER_REDIS_\{n\}_POOL_TIMEOUT           | ...pool.timeout               |
| STORAGE_PROVIDER_REDIS_\{n\}_POOL_CONN_MAX_IDLE_TIME| ...pool.conn_max_idle_time    |
| STORAGE_PROVIDER_REDIS_\{n\}_POOL_CONN_MAX_LIFETIME | ...pool.conn_max_lifetime     |

Event provider pools are configured in YAML only.
