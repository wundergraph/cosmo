...
|                      | compression                      | <Icon icon="square" />                        | [WebSocket Compression Configuration](/router/configuration#websocket-compression-configuration)                                             |               |
...
Configure permessage-deflate compression (RFC 7692) for WebSocket connections between clients and the Router. When enabled, the Router negotiates compression with clients that offer the extension during the connection upgrade. Clients that do not support compression are served uncompressed messages.

| Environment Variable                          | YAML                   | Required               | Description                                                                                                                                                                                             | Default Value |
| --------------------------------------------- | ---------------------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| WEBSOCKETS_COMPRESSION_ENABLED                | enabled                | <Icon icon="square" /> | Enable permessage-deflate compression for WebSocket connections.                                                                                                                                       | false         |
| WEBSOCKETS_COMPRESSION_LEVEL                  | level                  | <Icon icon="square" /> | The compression level (1-9). Level 1 is fastest with the least compression. Level 9 provides the best compression but is slowest.                                                                      | 6             |
| WEBSOCKETS_COMPRESSION_CLIENT_MAX_WINDOW_BITS | client_max_window_bits | <Icon icon="square" /> | Limits the LZ77 sliding window size (8-15) that clients may use when compressing messages. Smaller values reduce Router memory usage for decompression at the cost of compression ratio. The parameter is only included in the negotiation response when the client offers it. | 15            |
...
websocket:
  ...
  compression:
    # enable permessage-deflate compression for WebSocket connections
    enabled: false
    # the compression level (1-9)
    level: 6
    # limits the LZ77 sliding window size (8-15) that clients may use when compressing messages
