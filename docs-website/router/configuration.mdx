...
| on_error | <Icon icon="square" /> | Handle request-time authentication errors with `reject` (generic 401 response) or `continue` (proceed unauthenticated, subject to existing authorization checks). Includes credential extraction, JWT validation, and custom authenticator errors. Using `continue` requires credential validation before protected work. See [configuring JWT validation error handling](/router/authentication-and-authorization#configuring-jwt-validation-error-handling). | reject |
