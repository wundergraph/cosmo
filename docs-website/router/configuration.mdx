...
| CORS_MATCH_ORIGINS     | match_origins     | <Icon icon="square" /> | Go regular expressions matched against the entire origin in addition to `allow_origins`. Matching is case-insensitive by default. Separate patterns with semicolons in the environment variable. | []                            |
...
Each pattern must match the entire origin, including its scheme and optional port. Anchors such as `^` and `$` are optional. For example, `https://([a-z0-9-]+\.)*example\.com` allows `https://example.com` and `https://app.example.com`, but rejects `https://app.example.com.evil.com`.
