...
| CORS_CUSTOM_SCHEMES    | custom_schemes    | <Icon icon="square" /> | Additional URL schemes permitted in literal `allow_origins` entries. Include the `://` suffix.                                                | []                                   |
...
Entries in `allow_origins` containing `*`, such as `*.example.com`, bypass scheme validation and do not require `custom_schemes`.

To allow a literal origin with a custom URL scheme, add the scheme to `custom_schemes` and the full origin to `allow_origins`. Schemes must include the `://` suffix. Scheme validation is case-insensitive. Adding a scheme does not allow every origin using that scheme.
