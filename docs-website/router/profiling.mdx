...
The router integrates with [Grafana Pyroscope](https://grafana.com/oss/pyroscope/) for exactly this. When enabled, the router periodically collects profiles and pushes them to a Pyroscope server, where you can explore them in Grafana using [Explore Profiles](https://grafana.com/docs/grafana/latest/explore/simplified-exploration/profiles/) (formerly Profiles Drilldown). This works against a self-hosted Pyroscope instance or [Grafana Cloud Profiles](https://grafana.com/products/cloud/profiles/).
...
See the [Pyroscope configuration reference](/router/configuration#pyroscope-continuous-profiling) for all available options, including the profile types to collect, the upload rate, and mutex/block profiling rates.
...
The router always attaches the hostname and router version as tags, and you can add your own via the `tags` option to filter across instances, environments, or deployments.
