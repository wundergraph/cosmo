---
title: "Entity Cache (Experimental)"
description: "Cache federation entity fetches to avoid fetching from upstream subgraphs."
icon: "boxes-stacked"
---

<Danger>
  **Experimental feature.** Entity caching is in alpha and is subject to change, and should not be used in production environments.
</Danger>
...
<Info>
  Configuring the entity cache does not make anything cacheable on its own. Subgraphs opt in by sending `Cache-Control: public`. If nothing appears to be cached, check the subgraph response headers first.
</Info>
...
```yaml config.yaml
entity_cache:
  # Enable entity caching. Default: false.
  enabled: true
...
**`enable_multi_fetch` disables entity caching.** When `engine.enable_multi_fetch` is on, the router merges entity fetches to the same subgraph within one parallel wave into a single request. Merged fetches are not cached, and nothing reports that they were skipped. Do not enable both and expect entity caching.
...
```
INFO  Entity cache enabled  {"fallback_ttl": "30s", "storage_provider": "redis", "key_prefix": "cosmo_entity_cache:", "storage_provider_id": "my_redis"}
```

With the memory provider:

```
INFO  Entity cache enabled  {"fallback_ttl": "30s", "storage_provider": "memory", "max_entries": 2048}
```
...
```
WARN  Entity cache degraded, serving from the subgraph instead
```
