...
<Note>
  [Schema discovery](/router/mcp/schema-discovery/overview) adds three more built-in tools: `search_schema`,
  `get_symbols` and `generate_query`. They let a model search a large schema and generate a valid operation, without
  the full schema in its context. Use them instead of `get_schema` when your schema is large.
</Note>
