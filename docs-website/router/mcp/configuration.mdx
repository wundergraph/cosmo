...
Instructions are served to every MCP client, whether it connects via `server/discover` or the legacy `initialize` handshake.

Protocol version `2026-07-28` is advertised by default. In session-based mode (`session.stateless: false`), clients negotiate `2025-11-25` or older.
