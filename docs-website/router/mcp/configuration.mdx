...
A mapped scalar's schema states its GraphQL nullability. A non-nullable variable, such as `$filter: JSON!`, emits the mapped type alone: `"type": "object"`. A nullable variable, such as `$filter: JSON`, emits the type with `"null"`: `"type": ["object", "null"]`.
