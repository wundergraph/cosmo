...
| `scalar_mappings`             | Maps custom scalar type names to the JSON Schema type advertised in MCP tool input schemas. Allowed values: `string`, `integer`, `number`, `boolean`, `object`, `array`. Custom scalars without a mapping default to `string`. See [Custom Scalar Mappings](#custom-scalar-mappings). | -                |
...
Map-valued options use comma-separated `key:value` pairs. For example: `MCP_SCALAR_MAPPINGS=JSON:object,BigInt:integer`.
...
<Note>
  Top-level variables with an `object` type are always advertised as non-nullable in the generated schema. A nullable variable mapped to `object` therefore loses its `null` union in the tool's input schema.
</Note>
