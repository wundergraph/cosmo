...
<Info>
  Scopes in the JWT are read from the `scope` claim. The claim can be a **space-separated string**, per
  OAuth 2.0 convention, or a **JSON array** of strings. Some identity providers emit the array format.
  A claim that is absent or `null` grants no scopes. Any other shape is rejected with `401 invalid_token`.
  This includes an array holding a non-string member. Invalid arrays are never read partially.
</Info>
...
<Steps>
  ...
  <Step title="Retry on same session">
