...
<Warning>
  When you run more than one MCP server with OAuth, set `oauth.jwks[].audiences` on each server to that server's
  resource identifier. Without it, the router accepts a token minted for one server on every other server.
</Warning>
