...
<Note>
The records are large. Use a low `limit`.
</Note>
...
{
  "symbols": [
    { "coordinate": "enum:Mood", "record": { "Values": [{ "Name": "HAPPY" }, { "Name": "SAD" }] } }
  ]
}
```
...
<Note>
A response with no queries and one `unsatisfied` reason is a normal result. It is not an error. It tells you that your schema cannot answer the request.
</Note>
