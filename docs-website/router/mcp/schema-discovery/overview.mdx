...
<Note>
  Do not set `expose_schema` to `true` with schema discovery. `get_schema` returns the full schema, and that is the
...
<CardGroup cols={2}>
  ...
  <Card title="Guides" icon="list-check" href="/router/mcp/schema-discovery/guides">
    Find duplicate work, curate an operation into a tool, or ship one in a BFF.
  </Card>
