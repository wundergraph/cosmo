...
- **Built-in tools** provided by the server itself: `get_operation_info` for tool discovery, and the optional `get_schema` and `execute_graphql` for schema access and arbitrary queries.
