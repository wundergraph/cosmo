...
By default, the router returns `401 Unauthorized` when authentication fails. Set `authentication.jwt.on_error: continue` to continue unauthenticated instead. This is useful when a custom module or subgraph validates opaque OAuth tokens.
