...
`authentication.jwt.on_error` accepts `reject` (default) and `continue`. The default returns `401 Unauthorized` for invalid JWT credentials. Use `continue` when a custom module or subgraph validates opaque OAuth tokens.
...
This option does not ignore custom authenticator errors, malformed WebSocket payloads, or startup errors.
