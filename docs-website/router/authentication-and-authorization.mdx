...
With `continue`, the router ignores invalid tokens, unsupported header prefixes, and empty tokens. The original credential remains available to custom modules and [configured header forwarding](/router/proxy-capabilities/subgraph-request-header-operations).
