...
- `continue`: proceed as unauthenticated so a custom module or downstream service can validate credentials such as opaque OAuth tokens. Required authentication and field authorization still apply.
...
An ignored failure leaves `request.auth.isAuthenticated` false, `request.auth.type` empty, and claims and scopes unset. No `X-Authenticated-By` header is added for that failure. A valid credential from another configured source still authenticates the request normally. The original credential remains available to custom modules and configured header propagation.
