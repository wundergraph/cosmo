...
`authentication.jwt.on_error` accepts only `reject` (default) and `continue`. Other values prevent the router from starting.

With `reject`, invalid credentials return HTTP `401 Unauthorized`. For WebSocket initial-payload authentication, the router sends an error message and closes the connection.

Use `continue` for HTTP and WebSocket authentication when a custom module or subgraph validates opaque OAuth tokens.
...
With `continue`, invalid credentials, including unsupported header prefixes and empty tokens, leave the request unauthenticated. The original credential remains available to custom modules and [configured header forwarding](/router/proxy-capabilities/subgraph-request-header-operations).

<Warning>
  Your module or downstream service must validate the credential before allowing protected operations.
</Warning>

With `authorization.require_authentication: true`, the router still rejects unauthenticated requests. Fields protected by `@authenticated` or `@requiresScopes` also keep their authorization checks.

This option does not ignore custom authenticator errors, malformed WebSocket payloads, or startup errors. MCP OAuth is unaffected.
