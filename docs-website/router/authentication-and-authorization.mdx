...
### Configuring JWT validation error handling

Use `authentication.jwt.on_error` to control how the router handles request-time authentication errors:

- `reject` (default): return a generic `401 Unauthorized` response.
- `continue`: ignore invalid credentials for router authentication and proceed as unauthenticated. The original header is preserved for custom modules and configured forwarding to subgraphs. Required authentication and field authorization still apply.
...
With this configuration, an invalid JWT or opaque token does not cause a router authentication rejection. The header forwarding engine sends the original `Authorization` value, including its prefix, to subgraphs. Without a matching forwarding rule, the credential is not forwarded. Use `headers.subgraphs` to limit forwarding to specific subgraphs. See [subgraph request header operations](/router/proxy-capabilities/subgraph-request-header-operations).

<Warning>
  This option permits unauthenticated traffic after authentication fails. A custom module or downstream service must validate the original credential before protected work or side effects. Header presence alone does not authenticate a request.
</Warning>

An ignored failure leaves `request.auth.isAuthenticated` false, `request.auth.type` empty, and claims and scopes unset. No `X-Authenticated-By` header is added for that failure. A valid credential from another configured source still authenticates the request normally.

The policy applies to all errors returned by the request's authenticators, including JWT validation failures, unsupported header prefixes, empty tokens, missing or malformed WebSocket authentication payloads, and custom authenticator failures. It does not require credentials to be present.

`authorization.require_authentication: true` still rejects unauthenticated requests. Field authorization through `@authenticated` and `@requiresScopes` remains effective. Configuration and authenticator initialization errors are not ignored.

The setting applies to GraphQL HTTP headers, WebSocket handshake headers, and configured WebSocket initial-payload tokens. It does not affect MCP OAuth authentication. Setting `on_error: continue` does not emit a startup warning or warnings for ignored authentication failures.
...
By default, fields protected by [@authenticated](/federation/directives/authenticated) or [@requiresScopes](/federation/directives/requiresscopes) are authorized after the subgraph fetch and filtered out of the response. This means the router may fetch data the client is not authorized to see, only to remove it afterwards.

With pre-fetch field authorization enabled, the router authorizes all protected fields of an operation in a single batch before any subgraph fetch runs. Since the decisions are scope-based and independent of the returned data, the router can skip fetches for fields the client is not allowed to access instead of fetching and then filtering them.
