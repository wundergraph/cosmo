...
`authentication.jwt.on_error` accepts `reject` (default) and `continue` for both HTTP and WebSocket authentication. Use `continue` when a custom module or subgraph validates opaque OAuth tokens.
