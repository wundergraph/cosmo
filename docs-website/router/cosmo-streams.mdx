...
By default the router will start even if an event provider referenced by the execution config is unavailable. This happens in two cases:

- The provider is not defined under `events.providers`. The router reports an error such as `redis provider with ID my-provider is not defined`. This applies to NATS, Kafka, and Redis.
- The provider is defined but cannot be reached at startup (e.g. the broker is down). This applies to NATS and Redis. The Kafka client connects lazily, so a router with an unreachable Kafka broker still starts by default.

The router logs the error and disables only the fields backed by the unavailable provider. The rest of the graph keeps serving traffic. The two cases use distinct log messages and behave differently:
...
Set up an alert on the logged error to detect the misconfiguration or outage.

If instead you want the router to fail to start in such cases, set `skip_unavailable_providers: false` (or the `EVENTS_SKIP_UNAVAILABLE_PROVIDERS` environment variable).
...
The subjects/topics/channels argument of all event directives allows you to use templating syntax to render the destination from field arguments.
...
Variable expansion is supported for NATS subjects, Kafka topics, and Redis channels. Redis channels follow the same template-expansion path. When using Kafka topic templates, make sure the rendered Kafka topics already exist. The Router does not create Kafka topics automatically.
