...
Environment variables bypass the config schema validation. A `fallback_ttl` of zero or less under `all`, or under an enabled subgraph entry, fails startup either way, as does a `private_id` that does not compile.
