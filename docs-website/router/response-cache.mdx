...
An outer array whose length does not match the entities answered costs the whole response its tags, rather than being zipped as far as the shorter of the two. Either extension is consumed by the router and never forwarded to clients. A merged entity fetch under `engine.enable_multi_fetch` declares no tags, see [Limitations](#limitations).
