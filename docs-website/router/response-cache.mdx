...
3. `private` marks the response as belonging to one user. It is cached only under a key scoped to that user, and only when `private_id` is configured and resolves for the request. See [Private responses](#private-responses). Without one, `private` refuses caching.
...
response_cache:
  ...
  key_prefix: 'cosmo_response_cache:'
  # Expression yielding the id of the user a request acts for, so private
  # subgraph responses can be cached per user. See Private responses.
...
A subgraph whose answer depends on who is asking marks it `Cache-Control: private`. A shared cache must not serve that answer to anyone else, but the router can still cache it for the user it was produced for, so the same user asking again is served without a subgraph request.

To do that the router needs to know who the user is. `private_id` is an [expression](/router/configuration/template-expressions) evaluated once per request that yields the user's id, such as a JWT claim or a request header:
...
* A `private` response is stored under a key that includes the id, so each user has an entry of their own. A lookup by one user never finds another user's entry.
...
* `public` responses are unaffected. They are stored under the shared key and served to every user, whatever the id.
* A response served from a user's entry reaches the client with `Cache-Control: private` and the entry's remaining lifetime, so a browser or CDN in front of the router does not share it either.
* Invalidation by subgraph, type, or cache tag drops private entries like any other.

The id is digested with SHA-256 before it becomes part of the key, so raw ids never reach the store and two users cannot share a key. The expression must evaluate to a string. A value of any other type is reported through the response cache warning described under [Observability](#observability) and treated as no id.

<Warning>
  A `private` response is only as private as the id. An expression that yields the same value for different users, or one that can be set by the client itself, lets one user be served another user's entry. Prefer a verified claim over a plain header.
</Warning>
...
* A header whose value differs per caller, such as `Cookie` or `Authorization`, yields one variant per caller. Nobody is served another caller's body, but the entries are rarely reused. A subgraph whose answer is per user should say `private` instead.
* A subgraph that changes what it varies on replaces the record on its next miss. Variants stored under the old names are never served again and expire on their own TTL.
...
**Forwarded request headers reach the cache key only through `Vary`.** The key covers the request the router renders for the subgraph, which is the operation text and its variables. Headers added by header propagation are applied after that and never reach the key on their own. Two requests that differ only by a propagated header share one entry unless the subgraph names that header in `Vary`, as described under [Varying responses](#varying-responses). A subgraph that varies by request header without saying so must use `no-store`, or `private` together with a `private_id`. Omitting `public` does not prevent caching.
...
**A merged entity fetch is served per alias.** With `engine.enable_multi_fetch` on, entity fetches to the same subgraph within one parallel wave are merged into a single request with aliased `_entities` fields. The response is taken apart again on the way into the cache, so each alias stores one entry per entity, the same as the unmerged fetch it replaces would have. Each alias is then served all-or-nothing like a batch: a warm alias is left out of the next merged request, a cold one is fetched whole. When some aliases are warm and others are not, the router still sends the request for the cold ones, and the `Cache-Control` it reports for that fetch is capped at the life left on the warm ones. Only a request that finds every alias warm is a cache hit.

**A merged entity fetch keeps no declared cache tags.** `apolloEntityCacheTags` is one flat list with nothing to say which alias each entry belongs to, so the tags a subgraph declares for a merged response are dropped. The entries are still indexed by subgraph and type, and invalidation by either still reaches them. Invalidation by cache tag misses them until they expire.

**A failed merged request skips what depends on its warm aliases.** When the request for the cold aliases fails to reach the subgraph, the warm aliases keep their cached data and only the cold ones report the failure. Fetches that depend on the merged fetch are skipped as a whole, though, including those that depend only on a warm alias. Their fields stay `null` without an error of their own.
