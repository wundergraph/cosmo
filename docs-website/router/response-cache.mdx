...
A CDN in front of the router can cache whole responses and purge them by tag. Enable `cache_tag_header` and the router returns the tags of each response in a header the CDN reads. Ensure that you set the correct tag name, for example Cloudflare purges by `Cache-Tag`, Fastly by `Surrogate-Key`.
