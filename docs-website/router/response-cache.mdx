...
Entries leave the cache on their own, when the TTL expires. However users might want to actively invalidate the cached data. In order to do this the router provides an invalidation endpoint on a different port, which can get called to invalidate entries.
