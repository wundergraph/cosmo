...
description: "Cache fetches to avoid subsequent requests reetching from upstream subgraphs."
icon: "boxes-stacked"
---
...
**Root query fetches.** A root query fetch is the request that opens a query: what the router asks a subgraph for the root fields the client selected. The whole answer is one entry, keyed by the request that produced it — the operation text and every variable value — so any difference in either is a miss and the request goes to the subgraph as it would have. Only queries are cached; mutations and subscriptions never are.
