...
For a query using [`@defer`](/router/incremental-delivery), ART is progressive. The initial multipart result contains the primary trace so the playground can display it immediately. The result with `hasNext: false` contains the authoritative trace for the whole request. It also includes deferred groups and each request's `completed`, `error`, or `skipped` status. If a stream ends before that terminal result, the trace is incomplete.
