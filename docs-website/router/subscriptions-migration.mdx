...
See [Subscriptions — Using the "extensions" field](/router/subscriptions#using-the-extensions-field) for the current behavior.
...
See [Subgraph Error Propagation — Upstream WebSocket errors during subscriptions](/router/subgraph-error-propagation#upstream-websocket-errors-during-subscriptions) for the current behavior and example payloads.
...
<Warning>
  The old `websocket_client_poll_timeout`, `websocket_client_conn_buffer_size`, `websocket_client_read_timeout`, and `websocket_client_frame_timeout` keys have been removed from the config schema, and their `ENGINE_WEBSOCKET_CLIENT_*` environment variables are no longer recognized. One option — `websocket_client_write_timeout` — keeps its name but now applies to a different side of the router. Review each section below before upgrading.
</Warning>
