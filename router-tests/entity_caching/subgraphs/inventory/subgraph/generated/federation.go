...
//lint:file-ignore SA4004 gqlgen-generated federation resolver
...
func (ec *executionContext) resolveEntity(
	...
	switch typeName {
	case "Item":
		resolverName, err := entityResolverNameForItem(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Item": %w`, err)
		}
		switch resolverName {
		...
		case "findItemByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findItemByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindItemByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Item": %w`, err)
			}
...
func entityResolverNameForItem(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Item", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Item", ErrTypeNotFound))
			break
		}
