...
// TestExecutionPlanSchedulingPlumbing proves the EnableExecutionPlanScheduling
// config reaches the engine: with the flag on and query plans included in the
// response, a multi-subgraph operation must report a NESTED schedule tree
// (Parallel containing Sequences) instead of the legacy flat wave grouping.
func TestExecutionPlanSchedulingPlumbing(t *testing.T) {
	t.Parallel()

	testenv.Run(t, &testenv.Config{
		ModifyEngineExecutionConfiguration: func(cfg *config.EngineExecutionConfiguration) {
			cfg.EnableExecutionPlanScheduling = true
