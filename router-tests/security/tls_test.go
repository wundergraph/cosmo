...
func TestTLS(t *testing.T) {
	...
	t.Run("Can't connect to server securely because client certificate is not signed be the same authority", func(t *testing.T) {
		...
		testenv.Run(t, &testenv.Config{
			...
			cert, err := tls.LoadX509KeyPair(testenv.ResolvePath("testdata/tls/cert-2.pem"), testenv.ResolvePath("testdata/tls/key-2.pem"))
			require.NoError(t, err)

			caCert, err := os.ReadFile(testenv.ResolvePath("testdata/tls/cert-2.pem"))
			require.NoError(t, err)
