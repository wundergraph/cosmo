...
func TestJWTOnErrorForwarding(t *testing.T) {
	...
	// Ignore JWT validation failures for both credential sources.
	decoder, err := authentication.NewJwksTokenDecoder(t.Context(), zap.NewNop(), []authentication.JWKSConfig{{
		Secret:    "jwt-on-error-test-secret-at-least-32-bytes",
		Algorithm: "HS256",
		KeyId:     "test",
	}})
	...
	testenv.Run(t, &testenv.Config{
		RouterOptions: []core.Option{
			...
			// Let the subgraph echo the original credential and router authentication state.
		...
		ModifyWebsocketConfiguration: func(cfg *config.WebSocketConfiguration) {
			...
			// Export the payload credential to the header used by the propagation rule.
		...
		t.Run("HTTP forwards invalid credentials without authenticating", func(t *testing.T) {
			...
			// An invalid JWT must not prevent the operation from completing.
			require.Equal(t, http.StatusOK, res.Response.StatusCode)

			// The subgraph receives the credential unchanged, but the router
			// still considers the request unauthenticated.
		...
		t.Run("WebSocket forwards invalid credentials without authenticating", func(t *testing.T) {
			...
			// Supply the opaque credential through the connection's initial payload.
			require.NoError(t, testenv.WSWriteJSON(t, conn, testenv.WebSocketMessage{
				Type:    "connection_init",
				Payload: json.RawMessage(`{"Authorization":"Bearer opaque"}`),
			}))

			// JWT validation failure must not reject connection initialization.
			...
			// The exported credential reaches the subgraph unchanged without
