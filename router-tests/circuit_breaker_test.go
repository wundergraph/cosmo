...
func sendRequest(t *testing.T, xEnv *testenv.Environment, isSuccessRequest *atomic.Bool, isSuccess bool) {
	if isSuccessRequest != nil {
		isSuccessRequest.Store(isSuccess)
	}
	res, err := xEnv.MakeGraphQLRequest(testenv.GraphQLRequest{Query: `query employees { employees { id } }`})
	require.NoError(t, err)
	if isSuccess {
