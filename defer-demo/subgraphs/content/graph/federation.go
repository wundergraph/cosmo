...
func (ec *executionContext) resolveEntity(
	...
	case "Article":
		...
		case "findArticleBySlugAndLocale":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findArticleBySlugAndLocale(): %w`, err)
			}
			id1, err := ec.unmarshalNString2string(ctx, rep["locale"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 1 for findArticleBySlugAndLocale(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindArticleBySlugAndLocale(ctx, id0, id1)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Article": %w`, err)
			}
	...
	case "ContentItem":
		resolverName, err := entityResolverNameForContentItem(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "ContentItem": %w`, err)
		}
		switch resolverName {
		...
		case "findContentItemByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findContentItemByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindContentItemByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "ContentItem": %w`, err)
			}

			return entity, nil
		}
	case "Podcast":
		resolverName, err := entityResolverNameForPodcast(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Podcast": %w`, err)
		}
		switch resolverName {
		...
		case "findPodcastByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPodcastByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindPodcastByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Podcast": %w`, err)
			}
...
func entityResolverNameForArticle(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"locale\" for Article", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		return "findArticleBySlugAndLocale", nil
	}
	return "", fmt.Errorf("%w for Article due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForContentItem(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for ContentItem", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for ContentItem", ErrTypeNotFound))
			break
		}
		return "findContentItemByID", nil
	}
	return "", fmt.Errorf("%w for ContentItem due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForPodcast(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Podcast", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Podcast", ErrTypeNotFound))
			break
		}
		return "findPodcastByID", nil
	}
	return "", fmt.Errorf("%w for Podcast due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
