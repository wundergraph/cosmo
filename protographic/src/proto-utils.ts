 ...
 * Used for finding similar field names when validation fails
 *
 ...
 *
 * @param inputName - The field name that was not found
 * @param availableNames - List of valid field names to compare against
 * @returns The most similar field name, or null if no good match exists
 */
export function findSimilarFieldName(inputName: string, availableNames: string[]): string | null {
    let bestMatch: string | null = null;
    let bestDistance = Infinity;
    const maxDistance = Math.floor(inputName.length / 2); // Allow up to half the characters to be different

    for (const name of availableNames) {
        const distance = levenshteinDistance(inputName.toLowerCase(), name.toLowerCase());
