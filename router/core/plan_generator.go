...
func (pg *PlanGenerator) loadConfiguration(routerConfig *nodev1.RouterConfig, logger *zap.Logger, maxDataSourceCollectorsConcurrency uint) error {
	...
	// NetPollConfiguration was removed by the subscriptions overhaul; the new client
	// manages its own goroutines without netpoll, so there is nothing to configure here.
