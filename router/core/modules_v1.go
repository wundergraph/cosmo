...
func (r *moduleRegistry) registerMyModule(instance MyModule) {
	...
	if val := m.New(); val == nil {
		panic(fmt.Sprintf("MyModuleInfo.New must return a non-nil module instance: %s", m.ID))
	}
...
func (c *coreModuleHooks) cleanupCoreModuleHooks(ctx context.Context) error {
	...
	for _, moduleInstance := range c.moduleInstances {
		if err := moduleInstance.Cleanup(ctx); err != nil {
			errs = append(errs, fmt.Errorf("failed to cleanup module %s: %w", moduleInstance.MyModule().ID, err))
		}
	}
