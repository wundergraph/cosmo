...
// TestWsConnectionWrapper_PooledFlateIsolation verifies that the pooled flate
// writers and readers used by the no-context-takeover paths never leak
// sliding-window state between messages or connections. If a pooled writer
// were reused without a clean reset, a repeated payload would compress into
// back-references to the previous stream, producing different bytes that are
// not independently decompressible.
func TestWsConnectionWrapper_PooledFlateIsolation(t *testing.T) {
	noTakeoverMode := compressionMode{enabled: true, level: 6, clientWindowBits: 15}

	// Repetitive payload large enough that a leaked window would certainly
	// produce cross-stream back-references.
	payload := string(bytes.Repeat([]byte(`{"id":1,"details":{"forename":"Jens","surname":"Neuse"}},`), 11))

	readFrames := func(t *testing.T, raw []byte, count int) []ws.Frame {
		t.Helper()
		reader := bytes.NewReader(raw)
		frames := make([]ws.Frame, 0, count)
		for i := 0; i < count; i++ {
	...
	// decompressFresh inflates a frame with a brand-new reader and no
	// dictionary, so it only succeeds for self-contained streams.
	decompressFresh := func(t *testing.T, frame ws.Frame) []byte {
		t.Helper()
		decompressed, err := wsflate.DecompressFrame(frame)
		require.NoError(t, err, "frame is not an independently decompressible DEFLATE stream")
		return decompressed.Payload
	}

	t.Run("repeated writes produce identical self-contained streams", func(t *testing.T) {
		conn := newMockConn()
		wrapper, err := newWSConnectionWrapper(conn, 0, 0, noTakeoverMode)
		require.NoError(t, err)

		require.NoError(t, wrapper.WriteText(payload))
		require.NoError(t, wrapper.WriteText(payload))

		frames := readFrames(t, conn.getWrittenBytes(), 2)
		assert.Equal(t, frames[0].Payload, frames[1].Payload,
			"identical messages must compress to identical bytes; a difference means window state leaked between messages")
		for _, frame := range frames {
			assert.Equal(t, payload, string(decompressFresh(t, frame)))
		}
	})

	t.Run("no state leaks across connections sharing the pool", func(t *testing.T) {
		connA := newMockConn()
		wrapperA, err := newWSConnectionWrapper(connA, 0, 0, noTakeoverMode)
		require.NoError(t, err)
		require.NoError(t, wrapperA.WriteText(payload))

		// Wrapper B very likely receives A's pooled writer back.
		connB := newMockConn()
		wrapperB, err := newWSConnectionWrapper(connB, 0, 0, noTakeoverMode)
		require.NoError(t, err)
		require.NoError(t, wrapperB.WriteText(payload))

		frameA := readFrames(t, connA.getWrittenBytes(), 1)[0]
		frameB := readFrames(t, connB.getWrittenBytes(), 1)[0]
		assert.Equal(t, frameA.Payload, frameB.Payload,
			"connections must produce identical bytes for identical messages; a difference means window state leaked across connections")
		assert.Equal(t, payload, string(decompressFresh(t, frameB)))
	})

	t.Run("pooled reader decompresses sequential independent messages", func(t *testing.T) {
		conn := newMockConn()
		wrapper, err := newWSConnectionWrapper(conn, 0, 0, noTakeoverMode)
		require.NoError(t, err)

		messages := []string{`{"first":{"id":1,"details":{"forename":"Jens","surname":"Neuse"}}}`, `{"second":"other content entirely"}`}
		for _, msg := range messages {
			compressed, err := compressData([]byte(msg))
			require.NoError(t, err)
			frame := ws.NewFrame(ws.OpText, true, compressed)
			frame.Header.Rsv = ws.Rsv(true, false, false)
			frame.Header.Masked = true
		...
		for _, want := range messages {
			...
			require.NoError(t, wrapper.ReadJSON(&got))
			assert.JSONEq(t, want, string(got))
		}
	})
}

// BenchmarkWriteCompressedNoContextTakeover tracks the cost of the pooled
// no-context-takeover write path, which is what gorilla/websocket clients
// (always negotiating no_context_takeover) hit on every message.
func BenchmarkWriteCompressedNoContextTakeover(b *testing.B) {
	conn := newMockConn()
	wrapper, err := newWSConnectionWrapper(conn, 0, 0, compressionMode{enabled: true, level: 6, clientWindowBits: 15})
	...
	for i := 0; i < b.N; i++ {
		conn.writeBuf.Reset()
		if err := wrapper.WriteText(payload); err != nil {
			b.Fatal(err)
		}
	}
}
