...
func (p *OperationPlanner) preparePlan(ctx *operationContext) (*planWithMetaData, error) {
	...
	if p.trackUsageInfo {
		...
		out.argumentUsageInfo, err = graphqlschemausage.GetArgumentUsageInfo(&doc, p.executor.RouterSchema, &p.executor.PlanConfig)
		if err != nil {
			return nil, err
		}
	}
...
func (p *OperationPlanner) plan(opContext *operationContext, options PlanOptions) (err error) {
	...
	if skipCache {
		...
		if options.TrackSchemaUsageInfo {
			...
			opContext.inputUsageInfo, err = graphqlschemausage.GetInputUsageInfo(prepared.operationDocument, p.executor.RouterSchema, opContext.variables, &p.executor.PlanConfig)
			if err != nil {
				return err
			}
		}
	...
	if options.TrackSchemaUsageInfo {
		...
		opContext.inputUsageInfo, err = graphqlschemausage.GetInputUsageInfo(opContext.preparedPlan.operationDocument, p.executor.RouterSchema, opContext.variables, &p.executor.PlanConfig)
		if err != nil {
			return err
		}
	}
