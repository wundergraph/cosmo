...
func (r *Router) bootstrap(ctx context.Context) error {
	...
	if r.mcp.Enabled {
		...
		if r.mcp.RouterURL != "" {
			...
			routerGraphQLEndpoint, err = url.JoinPath(r.baseURL, r.graphqlPath)
			if err != nil {
				return fmt.Errorf("failed to construct MCP GraphQL endpoint URL: %w", err)
			}
		}
	...
	if r.connectRPC.Enabled {
		...
		if r.connectRPC.GraphQLEndpoint != "" {
			...
			routerGraphQLEndpoint, err = url.JoinPath(r.baseURL, r.graphqlPath)
			if err != nil {
				return fmt.Errorf("failed to construct ConnectRPC GraphQL endpoint URL: %w", err)
			}
		}
