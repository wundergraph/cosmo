...
func TestBuildPreambleGolden(t *testing.T) {
	got := buildPreamble("async () => ({ ok: true })")
...
const __agentMain = (async () => ({ ok: true }));
(async () => {
...
func TestBuildPreambleReportsUserCodeStartLine(t *testing.T) {
	got := userCodeStartLine(buildPreamble("async () => 1"))
