...
func TestExprManager(t *testing.T) {
	...
	t.Run("verify when GetRawBody is not called", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("verify when GetRawBody is called", func(t *testing.T) {
		t.Parallel()
		...
		_, err = exprManager.CompileAnyExpression("request.body.GetRawBody()")
		if err != nil {
			require.Fail(t, "unexpected error", err)
		}
