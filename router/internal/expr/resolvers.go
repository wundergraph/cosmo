...
func ResolveAnyExpression(vm *vm.Program, ctx ExpressionContext) (any, error) {
...
// provide the context for the expression evaluation. Not safe for concurrent use.
func ResolveStringExpression(vm *vm.Program, ctx ExpressionContext) (string, error) {
	r, err := expr.Run(vm, ctx)
	if err != nil {
		return "", handleExpressionError(err)
	}
...
// provide the context for the expression evaluation. Not safe for concurrent use.
func ResolveBoolExpression(vm *vm.Program, ctx ExpressionContext) (bool, error) {
	if vm == nil {
		return false, nil
	}
