...
// compileOptions returns the base compile options
func compileOptions(opts ...expr.Option) []expr.Option {
	baseOpts := []expr.Option{
		expr.Env(&Context{}),
...
// Compiler handles all expression compilation in a centralized way
type Compiler struct {
	// Cache for compiled expressions
	boolPrograms   sync.Map
	stringPrograms sync.Map
	anyPrograms    sync.Map
}
...
// CompileBool compiles a boolean expression, using cache if available
func (c *Compiler) CompileBool(s string, visitors ...ast.Visitor) (*vm.Program, error) {
	if prog, ok := c.boolPrograms.Load(s); ok {
		return prog.(*vm.Program), nil
	}
	opts := compileOptions(expr.AsBool())
	for _, visitor := range visitors {
		opts = append(opts, expr.Patch(visitor))
	}
	prog, err := expr.Compile(s, opts...)
	if err != nil {
		return nil, handleExpressionError(err)
	}
	c.boolPrograms.Store(s, prog)
	return prog, nil
}

// CompileString compiles a string expression, using cache if available
func (c *Compiler) CompileString(s string, visitors ...ast.Visitor) (*vm.Program, error) {
	if prog, ok := c.stringPrograms.Load(s); ok {
		return prog.(*vm.Program), nil
	}
	opts := compileOptions(expr.AsKind(reflect.String))
	for _, visitor := range visitors {
		opts = append(opts, expr.Patch(visitor))
	}
	prog, err := expr.Compile(s, opts...)
	...
	c.stringPrograms.Store(s, prog)
	return prog, nil
}

// CompileStringWithPatch compiles a string expression with a custom visitor
func (c *Compiler) CompileStringWithPatch(s string, visitor ast.Visitor) (*vm.Program, error) {
	// For expressions with patches, we don't use cache as the patch might be different
	opts := compileOptions(expr.AsKind(reflect.String), expr.Patch(visitor))
	prog, err := expr.Compile(s, opts...)
	if err != nil {
		return nil, handleExpressionError(err)
	}
...
// CompileAny compiles an expression that can return any type, using cache if available
func (c *Compiler) CompileAny(s string, visitors ...ast.Visitor) (*vm.Program, error) {
	if prog, ok := c.anyPrograms.Load(s); ok {
		return prog.(*vm.Program), nil
	}
	opts := compileOptions()
	for _, visitor := range visitors {
