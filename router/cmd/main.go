...
func Main() {
	...
	if cfg.Graph.RouterConfigPath != "" {
		routerConfig, err = app.SerializeConfigFromFile(cfg.Graph.RouterConfigPath)
		if err != nil {
			logger.Fatal("Could not read router config", zap.Error(err), zap.String("path", cfg.Graph.RouterConfigPath))
		}
	}
