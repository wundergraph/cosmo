...
func TestFindProtoFilesInDir(t *testing.T) {
	t.Run("finds proto files in directory", func(t *testing.T) {
		tmpDir := t.TempDir()

		// Create proto file
		require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "test.proto"), []byte("syntax = \"proto3\";"), 0644))
		// Create non-proto file
		require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "test.txt"), []byte("not a proto"), 0644))

		files, err := findProtoFilesInDir(tmpDir)
		require.NoError(t, err)
		require.Len(t, files, 1)
		assert.Contains(t, files[0], "test.proto")
	})

	t.Run("does not recurse into subdirectories", func(t *testing.T) {
		tmpDir := t.TempDir()

		// Create proto in root
		require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "root.proto"), []byte("syntax = \"proto3\";"), 0644))

		// Create subdirectory with proto
		subDir := filepath.Join(tmpDir, "sub")
		require.NoError(t, os.MkdirAll(subDir, 0755))
		require.NoError(t, os.WriteFile(filepath.Join(subDir, "sub.proto"), []byte("syntax = \"proto3\";"), 0644))

		files, err := findProtoFilesInDir(tmpDir)
		require.NoError(t, err)
		// Should only find root.proto, not sub.proto
		require.Len(t, files, 1)
		assert.Contains(t, files[0], "root.proto")
	})
}

func TestFindOperationFiles(t *testing.T) {
	t.Run("finds graphql files recursively", func(t *testing.T) {
		tmpDir := t.TempDir()

		// Create operations in root
		require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "root.graphql"), []byte("query Root { }"), 0644))

		// Create operations in subdirectories
		queriesDir := filepath.Join(tmpDir, "queries")
		mutationsDir := filepath.Join(tmpDir, "mutations")
		require.NoError(t, os.MkdirAll(queriesDir, 0755))
		require.NoError(t, os.MkdirAll(mutationsDir, 0755))

		require.NoError(t, os.WriteFile(filepath.Join(queriesDir, "query.graphql"), []byte("query Q { }"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(mutationsDir, "mutation.gql"), []byte("mutation M { }"), 0644))

		files, err := findOperationFiles(tmpDir)
		require.NoError(t, err)
		assert.Len(t, files, 3, "Should find all .graphql and .gql files recursively")
	})

	t.Run("finds both .graphql and .gql extensions", func(t *testing.T) {
		tmpDir := t.TempDir()

		require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "test.graphql"), []byte("query { }"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "test.gql"), []byte("query { }"), 0644))
		require.NoError(t, os.WriteFile(filepath.Join(tmpDir, "test.txt"), []byte("not graphql"), 0644))

		files, err := findOperationFiles(tmpDir)
		require.NoError(t, err)
		assert.Len(t, files, 2, "Should find both .graphql and .gql files")
	})
}

func TestExtractPackageFromProto(t *testing.T) {
	tests := []struct {
		name     string
		content  string
		expected string
		...
		{
			name: "extracts simple package",
			content: `syntax = "proto3";
package employee.v1;`,
			expected: "employee.v1",
		},
		{
			name: "extracts package with spaces",
			content: `syntax = "proto3";
package   employee.v1  ;`,
			expected: "employee.v1",
		},
		{
			name: "extracts nested package",
			content: `syntax = "proto3";
package com.example.employee.v1;`,
			expected: "com.example.employee.v1",
		},
		{
			name:     "returns empty for missing package",
			content:  `syntax = "proto3";`,
			expected: "",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := extractPackageFromProto(tt.content)
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestExtractServiceNameFromProto(t *testing.T) {
	tests := []struct {
		name     string
		content  string
		expected string
		...
		{
			name: "extracts service name with brace",
			content: `syntax = "proto3";
service EmployeeService {`,
			expected: "EmployeeService",
		},
		{
			name: "extracts service name without brace",
			content: `syntax = "proto3";
service EmployeeService`,
			expected: "EmployeeService",
		},
		{
			name: "extracts first service when multiple exist",
			content: `syntax = "proto3";
service FirstService {
}
service SecondService {
}`,
			expected: "FirstService",
		},
		{
			name:     "returns empty for missing service",
			content:  `syntax = "proto3";`,
			expected: "",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := extractServiceNameFromProto(tt.content)
			assert.Equal(t, tt.expected, result)
		})
	}
}