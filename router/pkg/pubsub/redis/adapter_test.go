...
func TestProviderAdapter_SubscribeWithoutStartupReturnsError(t *testing.T) {
	...
	adapter := NewProviderAdapter(ctx, zaptest.NewLogger(t), redisAdapterOptions{urls: []string{"redis://localhost:6379"}, clusterEnabled: false}, datasource.ProviderOpts{})
...
func TestProviderAdapter_ConnectionCleanupOnUnsubscribe(t *testing.T) {
	...
	ctx := context.Background()
	adapter := NewProviderAdapter(ctx, zaptest.NewLogger(t), redisAdapterOptions{urls: []string{fmt.Sprintf("redis://%s", redis.Addr())}, clusterEnabled: false}, datasource.ProviderOpts{})
