...
func TestSubscriptionEventUpdater_Done(t *testing.T) {
	// The subscriptions overhaul collapsed `Close(kind)` into `Done()`/`Error(data)`,
	// so the previous matrix over close kinds reduces to a single Done() expectation.
	mockUpdater := NewMockSubscriptionUpdater(t)
	config := &testSubscriptionEventConfig{
		providerID:   "test-provider",
		providerType: ProviderTypeNats,
