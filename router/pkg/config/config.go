...
type MCPOAuthConfiguration struct {
	...
	// ScopesSupported represents the minimal set of scopes required for basic functionality.
	// These scopes are advertised to MCP clients via the OAuth 2.0 Protected Resource Metadata endpoint
	// to inform clients which scopes they should request during authorization.
	ScopesSupported        []string            `yaml:"scopes_supported" env:"SCOPES_SUPPORTED"`
}
