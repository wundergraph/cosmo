...
func TestTransport(t *testing.T) {
	...
	t.Run("emits response body read span when enabled", func(t *testing.T) {
		content := []byte("Hello, world!")
		...
		r, err := http.NewRequestWithContext(context.Background(), http.MethodGet, ts.URL+"/test", nil)
		require.NoError(t, err)
		...
		tr := NewTransport(http.DefaultTransport, []otelhttp.Option{
			otelhttp.WithSpanOptions(trace.WithAttributes(otel.EngineTransportAttribute)),
			otelhttp.WithTracerProvider(&FilteringTracerProvider{TracerProvider: tp}),
		}, WithResponseBodyReadSpan())
		...
		var httpSpan, readSpan sdktrace.ReadOnlySpan
		for _, span := range sn {
			switch span.Name() {
			case "HTTP GET":
				httpSpan = span
			case "HTTP - Read Response Body":
				readSpan = span
			}
		}
		require.NotNil(t, httpSpan)
		require.NotNil(t, readSpan)
		require.Equal(t, httpSpan.SpanContext().SpanID(), readSpan.Parent().SpanID())
		require.True(t, readSpan.EndTime().After(readSpan.StartTime()))
		require.Contains(t, readSpan.Attributes(), otel.EngineTransportAttribute)
	})
}
