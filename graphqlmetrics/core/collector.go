...
// Option defines the method to customize Collector.
type Option func(s *Collector)
...
func NewCollector(ctx context.Context, metricsService graphqlmetricsv1connect.GraphQLMetricsServiceHandler, opts ...Option) *Collector {
	s := &Collector{
...
func (s *Collector) bootstrap(ctx context.Context) {
...
func (s *Collector) Shutdown(ctx context.Context) error {
	if s.server == nil {
		return errors.New("server not initialized")
	}
...
func (s *Collector) shutdownPrometheusServer(ctx context.Context) error {
	if s.prometheusServer == nil {
...
func (s *Collector) startPrometheusServer() error {
	if s.prometheusServer == nil {
		return errors.New("prometheus server was not initialized")
	}
