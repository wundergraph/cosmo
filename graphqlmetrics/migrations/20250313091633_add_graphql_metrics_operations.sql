...
CREATE TABLE IF NOT EXISTS graphql_metrics_operations
(
    ...
    INDEX idx_organization_id OrganizationID TYPE bloom_filter(0.001) GRANULARITY 1,
    INDEX idx_federated_graph_id FederatedGraphID TYPE bloom_filter(0.001) GRANULARITY 1,
    ...
    engine = ReplacingMergeTree(Timestamp) PARTITION BY toDate(Timestamp)
        ...
        ORDER BY (FederatedGraphID, OrganizationID, OperationHash, OperationName, OperationType)
...
DROP TABLE IF EXISTS graphql_metrics_operations;
