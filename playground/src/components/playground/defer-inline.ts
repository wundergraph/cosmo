...
export type InlineAdvisorGeneration = { identity: string; generation: number };

export const inlineAdvisorIdentity = ({
  tabId,
  query,
  operationName,
  variables,
  ...
  headers?: string | null;
}) => JSON.stringify([tabId ?? '', query ?? '', operationName ?? '', variables ?? '', headers ?? '']);

export const manualAdvisorContextIdentity = ({
  tabId,
  operationName,
  variables,
  headers,
  ...
  variables?: string | null;
  headers?: string | null;
}) => JSON.stringify([tabId ?? '', operationName ?? '', variables ?? '', headers ?? '']);

export const advanceInlineAdvisorGeneration = (
  current: InlineAdvisorGeneration,
  identity: string,
): InlineAdvisorGeneration =>
  current.identity === identity ? current : { identity, generation: current.generation + 1 };

export const isCurrentInlineAdvisorGeneration = (
  candidate: InlineAdvisorGeneration,
  current: InlineAdvisorGeneration,
) => candidate.generation === current.generation && candidate.identity === current.identity;
